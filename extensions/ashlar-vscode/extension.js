...
const PATCH_SCHEME = "ashlar-patch";
...
function activate(context) {
  ...
  statusBar.command = "ashlar.configureConnection";
  statusBar.text = "$(debug-disconnect) Ashlar";
  statusBar.tooltip = "Ashlar: Configure Connection";
  statusBar.show();
  context.subscriptions.push(statusBar);

  chatProvider = new ChatViewProvider(context.extensionUri);
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider("ashlar.chatView", chatProvider)
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("ashlar.checkConnection", () => checkConnection(true)),
    vscode.commands.registerCommand("ashlar.configureConnection", () => configureConnection()),
    vscode.commands.registerCommand("ashlar.openChat", () =>
      vscode.commands.executeCommand("ashlar.chatView.focus")
    ),
    vscode.commands.registerCommand("ashlar.askSelection", () => askSelection()),
    vscode.commands.registerCommand("ashlar.planWithPrompt", () => planWithPrompt()),
    vscode.commands.registerCommand("ashlar.editWithPrompt", () => editWithPrompt()),
    vscode.commands.registerCommand("ashlar.directorGoal", () => directorGoal()),
    vscode.commands.registerCommand("ashlar.cancelActiveRun", () => cancelActiveRun()),
    vscode.commands.registerCommand("ashlar.showRuns", () => showRuns()),
    vscode.commands.registerCommand("ashlar.showWorkloads", () => showWorkloads()),
    vscode.commands.registerCommand("ashlar.scaleWorkload", () => scaleWorkload()),
    vscode.commands.registerCommand("ashlar.applyAllPatches", () => applyAllPatches()),
    vscode.commands.registerCommand("ashlar.rejectAllPatches", () => rejectAllPatches()),
    vscode.commands.registerCommand("ashlar.undoLastApply", () => undoLastApply()),
    vscode.commands.registerCommand("ashlar.showPendingPatches", () => showPendingPatches()),
    vscode.commands.registerCommand("ashlar.refreshModels", () => refreshModels(true)),
    vscode.commands.registerCommand("ashlar.startLocalStack", () => startLocalStack()),
    vscode.commands.registerCommand("ashlar.applyPatch", (filePath) => applyOnePatch(filePath)),
    vscode.commands.registerCommand("ashlar.rejectPatch", (filePath) => rejectOnePatch(filePath)),
    vscode.commands.registerCommand("ashlar.diffPatch", (filePath) => openPatchDiff(filePath))
  );

  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (
        e.affectsConfiguration("ashlar.apiHost") ||
        e.affectsConfiguration("ashlar.apiPort") ||
        e.affectsConfiguration("ashlar.baseUrl") ||
        e.affectsConfiguration("ashlar.ollamaPort")
      ) {
  ...
  const pollMs = vscode.workspace.getConfiguration("ashlar").get("pollRunsMs", 3000);
  const runTimer =
    pollMs > 0
      ? setInterval(() => {
          refreshRuns().catch(() => {});
      ...
      : null;
  const workloadMs = vscode.workspace.getConfiguration("ashlar").get("pollWorkloadsMs", 10000);
  const workloadTimer =
    workloadMs > 0
      ? setInterval(() => {
          refreshWorkloads().catch(() => {});
...
function getConnection() {
  const cfg = vscode.workspace.getConfiguration("ashlar");
 ...
 * Clears ashlar.baseUrl so apiHost/apiPort take effect.
...
async function applyConnectionSettings(opts) {
  ...
  if (opts.notify !== false) {
    vscode.window.showInformationMessage(`Ashlar API set to ${base}`);
  }
  chatProvider?.postConnectionSettings(getConnection());
...
async function configureConnection() {
  ...
  const host = await vscode.window.showInputBox({
    prompt: "Ashlar agent-server host",
  ...
  const portStr = await vscode.window.showInputBox({
    prompt: "Ashlar agent-server HTTP port (ASHLAR_AGENT_SERVER_HTTP_PORT)",
  ...
  const ollamaPortStr = await vscode.window.showInputBox({
    prompt: "Host Ollama port (ASHLAR_OLLAMA_HOST_PORT)",
  ...
  try {
    ...
    vscode.window.showErrorMessage(`Ashlar connection settings: ${err.message}`);
  }
}
...
function updateBusyStatus() {
  ...
  if (activeRun) {
    statusBar.text = `$(sync~spin) Ashlar :${conn.port} ${activeRun.kind}`;
  ...
  if (statusBar && !String(statusBar.text).includes("debug-disconnect")) {
    statusBar.text =
      pending > 0
        ? `$(git-pull-request) Ashlar :${conn.port} (${pending})`
        : `$(check) Ashlar :${conn.port}`;
  }
}
...
async function scaleWorkload(workloadId) {
  ...
  if (!lastWorkloads || !lastWorkloads.workloads.length) {
    vscode.window.showWarningMessage(
      lastWorkloads
        ? `No workloads for provider '${lastWorkloads.provider}'. Configure Ashlar:WorkloadScaling.`
...
async function cancelActiveRun() {
  ...
  if (!run) {
    ...
    const running = lastRuns.find((r) => r.status === "running");
    if (!running) {
      vscode.window.showInformationMessage("No active Ashlar run.");
      return;
    }
    try {
...
async function showRuns() {
  ...
  const picked = await vscode.window.showQuickPick(
    ...
    { placeHolder: "Ashlar run timeline" }
  );
  if (!picked) return;
...
async function checkConnection(notify) {
  try {
    ...
    if (notify) {
      vscode.window.showInformationMessage(`Ashlar connected (${ollama}) at ${conn.baseUrl}`);
    }
    ...
    if (notify) {
      vscode.window.showWarningMessage(
        `Ashlar offline at ${getBaseUrl()}: ${err.message}. Start the agent-server or run "Ashlar: Start Local Stack".`
      );
    }
...
async function refreshModels(notify = false) {
  try {
    ...
    if (notify) {
      vscode.window.showInformationMessage(
        `Ashlar models: ${(models.models || []).length}, agents: ${lastAgents.length}`
      );
    }
  } catch (err) {
    if (notify) {
      vscode.window.showErrorMessage(`Failed to refresh Ashlar catalog: ${err.message}`);
    }
  }
}
...
async function askSelection() {
  ...
  const question = await vscode.window.showInputBox({
    prompt: "Ask Ashlar about the selection",
...
async function runChat(prompt, mode, modelId, agentId) {
  ...
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "Ashlar chat…", cancellable: true },
    async (_prog, token) => {
      ...
      try {
        ...
        await refreshRuns();
        await vscode.commands.executeCommand("ashlar.chatView.focus");
      } catch (err) {
        ...
        if (useStream && (err.statusCode === 404 || /404|Not Found/i.test(String(err.message)))) {
          try {
            ...
            vscode.window.showErrorMessage(`Ashlar chat failed: ${err2.message}`);
            return;
          }
        }
        vscode.window.showErrorMessage(`Ashlar chat failed: ${err.message}`);
      }
    }
  );
}
...
async function runPlan(prompt, modelId, agentId) {
  const cfg = vscode.workspace.getConfiguration("ashlar");
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "Ashlar planning…", cancellable: true },
    async (_prog, token) => {
      token.onCancellationRequested(() => cancelActiveRun());
      try {
        const body = {
        ...
        vscode.window.showErrorMessage(`Ashlar plan failed: ${err.message}`);
      }
    }
  );
}
...
async function runDirector(prompt, modelId, agentId) {
  const cfg = vscode.workspace.getConfiguration("ashlar");
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "Ashlar director…", cancellable: true },
    async (_prog, token) => {
      token.onCancellationRequested(() => cancelActiveRun());
      try {
        const body = {
        ...
        if (String(err.message || err).includes("cancelled") || err.code === "ECONNRESET") return;
        vscode.window.showErrorMessage(`Ashlar director failed: ${err.message}`);
      }
    }
  );
}
...
async function runEdit(prompt, modelId, agentId) {
  const cfg = vscode.workspace.getConfiguration("ashlar");
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "Ashlar proposing edits…", cancellable: true },
    async (_prog, token) => {
      token.onCancellationRequested(() => cancelActiveRun());
      try {
        const body = {
        ...
        if (!patches.length) {
          vscode.window.showWarningMessage("Ashlar returned no safe patches.");
        ...
        const choice = await vscode.window.showInformationMessage(
          `Ashlar proposed ${patches.length} file patch(es).`,
          "Apply All",
          "Review Diffs",
          "Reject All"
        );
        ...
        else await vscode.commands.executeCommand("ashlar.chatView.focus");
      } catch (err) {
        if (String(err.message || err).includes("cancelled") || err.code === "ECONNRESET") return;
        vscode.window.showErrorMessage(`Ashlar edit failed: ${err.message}`);
      }
    }
  );
}
...
async function openPatchDiff(filePath) {
  ...
  await vscode.commands.executeCommand(
    ...
    leftUri,
    proposedUri,
    `Ashlar: ${filePath}${patch.summary ? ` — ${patch.summary}` : ""}`
  );
}
...
async function showPendingPatches() {
  ...
  if (!pending.length) {
    vscode.window.showInformationMessage("No pending Ashlar patches.");
    return;
  }
  const picked = await vscode.window.showQuickPick(
    pending.map((p) => ({
    ...
    { placeHolder: "Pending Ashlar patches" }
  );
...
async function applyAllPatches() {
  ...
  if (ok) {
    ...
    updatePendingStatus();
    vscode.window.showInformationMessage(`Applied ${pending.length} patch(es). Use "Ashlar: Undo Last Apply" to revert.`);
  }
}
...
async function startLocalStack() {
  ...
  if (!script || !fs.existsSync(script)) {
    vscode.window.showErrorMessage(
      "Start script not found. Set ashlar.startStackScript or open the Ashlar repo workspace."
    );
    return;
  }
  ...
  const terminal = vscode.window.createTerminal({ name: "Ashlar Stack" });
  terminal.show();
  ...
  vscode.window.showInformationMessage(
    `Starting Ashlar local stack on ${conn.host}:${conn.port} (ollama host port ${ollamaPort})…`
  );
    ...
    webviewView.webview.onDidReceiveMessage(async (msg) => {
      if (msg.type === "chat") {
        ...
        try {
          ...
          vscode.window.showErrorMessage(`Ashlar port/host: ${err.message}`);
        }
...
<body>
  <div id="status" class="status bad">Checking Ashlar…</div>
  <div class="conn" title="Agent-server host and HTTP port (ASHLAR_AGENT_SERVER_HTTP_PORT)">
  ...
  <div id="patches"></div>
  <div id="log"></div>
  <textarea id="prompt" placeholder="Ask, plan, edit, or director with Ashlar…"></textarea>
  <div class="row">
