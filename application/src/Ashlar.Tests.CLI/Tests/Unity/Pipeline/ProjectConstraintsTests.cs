...
{
    private readonly string _tempDir = Path.Combine(Path.GetTempPath(), $"ashlar-constraints-{Guid.NewGuid():N}");
    ...
    public void SaveToFile_CreatesFile()
    {
        ...
        var path = Path.Combine(_tempDir, ".ashlar", "constraints.json");
