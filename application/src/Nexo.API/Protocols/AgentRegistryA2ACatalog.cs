...
/// deliberately cannot reference <c>Nexo.Core.Domain</c> (transport layering rules) — this is
/// the one place the domain persona model and the A2A descriptor meet.
/// </summary>
public sealed class AgentRegistryA2ACatalog : INexoA2AAgentCatalog
{
    ...
    public IReadOnlyList<NexoA2AAgentDescriptor> GetAgents()
    {
        ...
        var descriptors = new List<NexoA2AAgentDescriptor>(agents.Count);
        foreach (var card in agents)
        {
            descriptors.Add(new NexoA2AAgentDescriptor(
                Id: card.Id,
                Name: card.Name,
