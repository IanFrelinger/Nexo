...
/// .ashlar/constraints.json in the project root. Constraints are injected
...
public sealed record ProjectConstraints
{
    ...
    /// <summary>Loads constraints from <c>.ashlar/constraints.json</c> under the project root.</summary>
    public static ProjectConstraints LoadFromFile(string projectRoot)
    {
        var path = Path.Combine(projectRoot, ".ashlar", "constraints.json");
        if (!File.Exists(path))
    ...
    /// <summary>Persists constraints to <c>.ashlar/constraints.json</c> under the project root.</summary>
    public void SaveToFile(string projectRoot)
    {
        var dir = Path.Combine(projectRoot, ".ashlar");
