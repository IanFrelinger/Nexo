...
internal static class UnityDevCommandUtilities
{
    ...
    *.mdb.meta
    ...
    ");
        ...
        var ashlarConfigPath = Path.Combine(fullPath, ".ashlar", "config.json");
        if (!File.Exists(ashlarConfigPath))
        {
            File.WriteAllText(ashlarConfigPath, JsonSerializer.Serialize(new
            {
                provider = "ollama",
                model = "llama3.1"
            }, new JsonSerializerOptions { WriteIndented = true }));
        ...
        createdFiles.Add(".ashlar/config.json");
        ...
        else
        {
            ...
            Console.WriteLine($"  .gitignore for Unity + Ashlar");
            Console.WriteLine($"  .ashlar/config.json (default: Ollama provider)");
            Console.WriteLine();
            Console.WriteLine("Next: open this folder in Unity Hub, or run:");
            Console.WriteLine($"  ashlar unity-dev generate --project-root {projectRoot} --system \"your system description\"");
        }
    ...
    {
        var existing = Environment.GetEnvironmentVariable("ASHLAR_PATH_ALLOWLIST_EXTRA") ?? "";
        ...
        Environment.SetEnvironmentVariable("ASHLAR_SANDBOX_ROOT", projectRoot);
        Environment.SetEnvironmentVariable("ASHLAR_PATH_ALLOWLIST_EXTRA", string.Join(",", paths));
    }
