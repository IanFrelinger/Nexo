...
internal static class DogfoodAllCommand
{
    ...
    public static async Task<int> ExecuteAsync(bool json, bool verbose = false)
    {
        ...
        {
            if (json)
                Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(new { passed = false, reason = "Not in Ashlar repo" }));
            else
                Console.Error.WriteLine("dogfood all FAILED: Not in Ashlar repo. Run from Ashlar repository root.");
            return 1;
        }
