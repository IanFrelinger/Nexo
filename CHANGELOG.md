...
- **`ashlar test local` fail-closes on a silent empty match.** A filter that discovers zero tests now exits `ValidationFailed` instead of 0. `workflow-regression-gate` keeps a JSON `TotalTests` floor as belt-and-suspenders.
- **Tester background agents fail-close on a silent empty match.** `TestRunRunnerAdapter` no longer reports `Success=true` when `TotalTests` is 0. The registry publishes Error severity for that empty run so the planner cannot treat skip-and-pass as green.
- **Brick / self-improve regression fail-closes on a silent empty match.** `DotNetRegressionTestRunner` no longer reports `AllPassed=true` when `dotnet test` executes zero tests.
- **`ashlar validate` fail-closes when no tests ran.** A directory with no test projects, an empty TRX, or `TestsRun == 0` is `Passed=false` instead of a skipped green result.
- **Planner `dotnet.test` / `forge.test` fail-close on a silent empty match.** Exit 0 with no available tests, no filter matches, or `Passed: 0` is `ok=false` so the planner cannot treat skip-and-pass as green.
- **`ashlar test parallel` fail-closes on a silent empty match.** `DotNetInstanceSpawner` no longer marks an instance `Passed` when `dotnet test` executes zero tests. Empty result lists no longer vacuous-pass in `ResultCollector` or `InstanceResultAggregator`.
- **`ashlar test portable --scope smoke` fail-closes on a silent empty match.** A filter that discovers zero tests now exits `ValidationFailed` instead of 0. `--filter` overrides the default `BaseFrameworkSmokeTests` slice.
- **Multi-platform `ashlar test` fail-closes on a silent empty match.** Native and container result aggregation no longer treat `dotnet test` exit 0 with zero executed tests as a platform pass. The iOS/container helpers no longer vacuous-pass when `total == 0`.
- **Standalone brick authoring verify fail-closes on a silent empty match.** `verify-standalone-brick-authoring.sh` wraps the generated `SampleThingBrick.Tests` run with `run-dotnet-test-counted.py` (`--min-tests 1`) so a scaffold that emits no tests cannot pass the distribution-matrix job.
- **UAT hello-brick and certification-teeth runs fail-close on a silent empty match.** `tests/uat/tier4.sh` and `tier0-2.sh` refuse `dotnet test` exit 0 when the log shows no executed tests, so the unfiltered `uat-gate` cannot skip-and-pass.
- **`ashlar dogfood` fail-closes on a silent empty match.** Blocks 2–9, `closedloop`, and `phasef` no longer treat `dotnet test` exit 0 with zero executed tests as a gate pass. `--filter` overrides the block's default slice.
- **`ashlar test multi-env` fail-closes on a silent empty match.** Framework, adaptation, trust, caching, and persistence suites no longer treat `dotnet test` exit 0 with zero executed tests as an environment pass. Framework `infrastructure-stress` failures are no longer ignored.
- **Makefile dogfood blocks fail-close on a silent empty match.** `dogfood-block1`–`9`, `closedloop`, and `phasef` wrap `run-dotnet-test-counted.py` so a missing class cannot skip-and-pass.
- **`make meai-pipeline-gate` fail-closes on a silent empty match.** Kernel Gate Tier A wraps `Ashlar.Tests.AI.Pipeline` with `run-dotnet-test-counted.py` (`--min-tests 43`). PRs that touch `src/Ashlar.AI.Pipeline` now trigger the gate.
- **`ashlar ci verify` fail-closes on a silent empty match.** ProdStyle and `BaseFrameworkSmokeTests` now use `run-dotnet-test-counted.py` (`--min-tests 123` / `9`) so `make ci-verify` cannot skip-and-pass when a filter matches nothing.
- **`background-agent proposals test` fail-closes on a silent empty match.** The operator check now uses the same `DotnetTestTool.Succeeded` predicate as apply `--verify-test`, so a project with no tests cannot skip-and-pass.
- **`make test-prime-time` fail-closes on a silent empty match.** The PrimeTime ProdStyle slnf (365 unique identities) now uses `run-dotnet-test-min-floor.py` so a filter that lists nothing cannot skip-and-pass.
- **`make test-prime-time-full` and `make test-framework-prod-first` fail-close on a silent empty match.** The remaining slnf slices (3627 / 2277 unique non-ProdStyle identities) now use the same min-floor helper.
- **`make test` and the local `test-all-platforms` sln pass fail-close on a silent empty match.** `Ashlar.sln` (4472 unique identities) now uses `run-dotnet-test-min-floor.py`.
- **`make test-mesh-lab` fail-closes on a silent empty match.** The MeshLab opt-in identity now uses `run-dotnet-test-counted.py` (`--min-tests 1`).
- **`make test-all-platforms` Docker smoke fail-closes on an empty TRX.** After each container run the host asserts `ubuntu` / `alpine` / `debian` smoke TRX executed ≥ 9 (`BaseFrameworkSmokeTests`). Images may lack Python, so the in-container invocation stays raw.
- **Trust multi-env Docker jobs fail-close on an empty TRX.** After each Ubuntu / Alpine / Debian mount the host asserts infra and background-agent Trust TRX executed ≥ 1. Dispatch-only; images may lack Python.
- **Cross-platform tests fail-close on a missing or empty TRX.** After the selected scope writes results, the host refuses zero executed tests. Dispatch-only; stays muted on push because of issue #252.
- **`ashlar self-extend` fail-closes on a silent empty match.** Preflight discoverability and generated-extension QA no longer treat `TotalTests` 0 as success when `--allow-mock` is set (the default).
- **`scripts/test-in-container.ps1` fail-closes on an empty TRX.** The in-container `dotnet test` stays raw (images may lack Python). After the run the host asserts `in-container.trx` executed ≥ 1.
- **Compose test lane fail-closes on an empty TRX.** After `docker-compose.test.yml` writes results, the host asserts `ubuntu-baseframework.trx` executed ≥ 9 (`BaseFrameworkSmokeTests`). Stays push/dispatch.
- **`test portable`, `test multi-env`, and `docker` honor `--format-json`.** They now use `CommandExecutionSupport.WantsJson` (alias lookup). The previous `o.Name == "--format-json"` test matched nothing because System.CommandLine strips the leading `--`.
- **`test portable`, `test multi-env`, and `docker` honor `--verbose`.** They now use `CommandExecutionSupport.WantsVerbose`. The previous `o.Name == "--verbose"` test matched nothing for the same reason.
- **CI trigger inventory matches the workflow files.** `docs/CiGateInventory.md` and `.github/workflows/README.md` now count 35 `pull_request` workflows (including `grpc-transport-gate`) and 13 dispatch-only workflows. `composition-mesh-gate` is not manual-only. A live counter pins the counts.
...
- **Kernel / ship / DR local gates match Production Readiness Gate v1.** Unconfigured `pipeline run` and LiteDB resume must stay `Failed`; they no longer require fabricated `ok=true` / `Completed`. `scripts/lib/assert-pipeline-fail-closed.py` is the shared assertion used by the canonical workflow and the three local mirrors.
- **Versioned publish is fail-closed on READY.** `release.yml` and `release-nuget.yml` require an autonomous release-manager `ready` verdict for the same SHA before images or NuGet jobs start. Reusable publish workflows pin Actions to commit SHAs. Optional SBOM/Grype install uses pinned release tarballs with SHA-256 checks instead of `curl | sh` from `main`.
...
- **`Ashlar.sln` Release builds enroll Fleet Contracts and Infrastructure** so commercial Fleet test references cannot resolve Debug assemblies. `LiteDbMeshTaskRegistry.TryGetByIdempotencyKeyAsync` takes the same store lock as the other registry methods.
...
- **Ollama construction no longer blocks the caller.** `OllamaProvider` does not pull `/api/tags` in the constructor, and `IsProviderAvailable("ollama")` reads the last refresh instead of `.GetAwaiter().GetResult()` on ASP.NET request threads. Chat and health refresh asynchronously.
- **Fleet.Host is in `Ashlar.sln` and a lane.** `Ashlar.Commercial.Fleet.Api`, `Fleet.Host`, and `Tests.Fleet.Host` enroll in the solution. `composition-mesh-gate-tier-c` runs the Host endpoint suite on net10.0 with a counted floor of 4, so those tests cannot rot unowned again.
- **Remaining sln-only suites now have counted runners.** MeshDirector (4) rides `composition-mesh-gate-tier-c`. Analyzer triads (56) append to the required `cert-gate`. AwsSns (11) and DynamoDb (2) run in `ingress-unit-gate` from `ashlar-ready-gate`. The brick template suite stays UNOWNED (scaffolding placeholders).
- **Ollama compose stacks pin a versioned digest.** `ci/ollama-image` is `ollama/ollama:0.33.3` plus the multi-arch index digest. `ASHLAR_OLLAMA_IMAGE` still overrides. `:latest` and the untagged `ollama/ollama` default are gone from `deploy/compose`.
...
- **Required cert-gate runs the full contracts suite.** `Ashlar.Tests.Contracts` is no longer release-only: cert-gate enforces the same 18 unique identities the tests lane requires. `products-gate` still runs the 5 Distributed tests on product-split paths.
- **Release-proof filtered gates are counted.** Composition-mesh Tier A (64) and B (3), plus security Tier A (97) and C (61), use `run-dotnet-test-counted.py`. A stale filter that matches zero can no longer pass.
- **Kernel Tier A and Ship Tier A host-DI smoke are counted.** `scripts/kernel-gate-tier-a.sh` floors hosting/E2E (40) and pipeline lifecycle (14). Ship Tier A floors the two named DI facts. The counted wrapper treats xUnit theory rows (`Method(data)`) as covering the listed method identity. `docs/OpenCoreBoundary.md` census is 108/100/8/21. Autonomy objectives write under `samples/dogfood/` instead of the removed `applications/` tree.
- **Application Tier C no longer vacuous-passes on net8.** The in-process API / virtual-prod demo suite is net10.0-only; the old `dotnet test -f net8.0` matched zero tests. The PR workflow now runs counted Tier C (`--min-tests 4`). Kernel B (14), Ship B smoke (9), and Security E (52 on net10, including air-gapped API host) are counted too.
- **Mesh Tier C fleet suite is counted.** The in-process commercial Fleet project (176 unique identities on net8.0) used a raw project run that still exits 0 on a silent empty match. The PR `composition-mesh-gate` path now wraps it with `--min-tests 176`.
- **`make test-prod-style` is counted.** The Infrastructure ProdStyle filter (123 unique identities on net8.0) used a raw `dotnet test` that still exits 0 on a silent empty match. Ship B and Kernel C call this target.
...
- **Security Gate Tier E host suite runs on pull requests.** The counted AirGapped|Safety slice (53 on net10) used a dispatch-only `if:` and never ran automatically. The `--network none` container proof stays dispatch-only.
- **`grpc-transport-gate` is counted and runs on pull requests.** Docs already called it a PR gate, but the workflow was push/dispatch only and used a raw `Category=ProdStyle` filter that still exits 0 on a silent empty match. Floor is 81 on net8.0. Kernel C calls the same script.
- **Kernel Gate Tier C is counted and runs on pull requests.** WorkflowExecutor (12) and AirGapped (18 on net10, including the API host facts net8 omitted) use the counted wrapper. The workflow header already claimed A–C; the previous dispatch-only `if:` never ran C automatically.
- **`ingress-unit-gate` runs on pull requests.** Counted AwsSns (11) and DynamoDb (2) units previously ran only from `make ashlar-ready-gate` / `make ingress-unit-gate` and had no GitHub workflow, so a PR that broke those suites stayed green.
- **Application B and Ship B fail-close `doctor --json`.** A non-zero doctor used to print PASS (Ship B silently, Application B unless `APPLICATION_GATE_STRICT_DOCTOR=1`). Both tiers now fail locally and on CI. Demo-profile doctor still treats Docker as warn-only (`ok` does not include `containerSmoke`).
- **Required cert-gate collapse floor is 447.** The product filter listed 447 rows after excluding convention tests; the previous floor of 400 let 47 listed rows vanish before the only merge-blocking check failed. The live TRX expected count stays derived from `--list-tests`.
- **`onboarding-docs-guard` runs on pull requests.** The startup-doc greps, referenced-path existence check, and ProjectTiers census previously ran only on push, so a PR that deleted a documented script or added a `.csproj` without a `docs/ProjectTiers.md` row stayed green until after merge.
- **`e2e-loop` has a collapse floor.** Measured 143 scenarios on Linux (137 without the six FIFO/symlink claims). The portability-gate sweep used to treat that tally as a scale, so deleting claims still exited 0 if the remainder passed.
- **`pack-hosting-graph-alignment` runs on pull requests.** The pack-list vs `Ashlar.Hosting` MSBuild graph check previously ran only on push to `master`/`main`. `distribution-matrix-gate` already invoked the same script on a narrower path set; the dedicated workflow's `src/**/*.csproj` filter now fires on PRs too.
- **`onboarding-quickstart-gate` native lane runs on pull requests.** `setup.sh check` / `restore`, CLI `--help`, and `doctor --json` previously ran only on push/schedule. The GHCR `:latest` container pull stays off pull_request.
- **`environment-setup-gate-v1` native 3-OS matrix runs on pull requests.** Linux/macOS `setup.sh` and Windows `setup.ps1` check/restore plus CLI build previously ran only on push to `master`/`main`. The MCR SDK container pull stays off pull_request.
- **`optimize-agent-cluster-gate` runs on pull requests.** Script-interface, bootstrap, scaffold/optimize, daemon-launch, and flag-combo jobs previously ran only on push, so a PR that broke `optimize_agent_cluster.sh` stayed green.
- **`runtime-release-gate` core and visual lanes run on pull requests.** The documented release quality bar (`runtime release-gate --allow-mock`) previously ran only on push to `master`/`main`. Chaos stays `continue-on-error`.
- **`installer-bruteforce-gate` runs on pull requests.** The 12-case host matrix (setup/bootstrap syntax, help, fail paths, CLI restore/build) previously ran only on `workflow_dispatch`, so a PR that broke `setup-linux.sh` or container-bootstrap dry-run stayed green.
- **RC Tier D refuses a missing or unauthenticated GitHub CLI.** `rc-gate-tier-d.sh` used to print skip and exit 0 when `gh` was absent or `gh auth status` failed, so `make rc-gate-tier-d` was a vacuous pass. It now exits 2. The RC Gate workflow sets `GH_TOKEN` on the Tier D step.
- **RC Tier C fails when the release bundle is missing or not PASS.** The evidence audit used to record `fail=1` and still print PASS unless `RC_GATE_STRICT_EVIDENCE=1`. The RC Gate workflow now produces `ci release-bundle --profile quick` before Tier C so the automatic job has a real artifact to audit.
- **Ship Tier D always runs `ci release-bundle`.** The default path used to skip the bundle and print PASS after doctor-only sign-off unless `SHIP_GATE_RUN_RUNTIME_GATE=1`. Doctor is still covered inside the bundle.
