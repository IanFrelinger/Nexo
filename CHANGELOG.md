...
- **Mutation leg: bounded mutant execution** (landing in `0.1.2`). A mutant that loops forever no longer hangs the certifier: each mutant's witness replay runs under a bound and a timed-out mutant is scored KILLED with the bound named in the reason, so a hostile or accidental non-terminating rewrite cannot stall the gate.
...
- **Mutation leg: cap/scope removal** (landing in `0.1.2`). The mutant cap and the member-scope filter that let a variant past the leg — a brick whose arithmetic sat beyond the cap, or in a `static` / `internal` member the filter skipped — are removed; every mutable site the catalog names is generated and must be killed.
