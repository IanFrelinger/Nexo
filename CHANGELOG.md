...
- **Application B and Ship B fail-close `doctor --json` on CI.** A non-zero doctor used to print PASS (Ship B silently). The PR workflows now set `APPLICATION_GATE_STRICT_DOCTOR=1` / `SHIP_GATE_STRICT_DOCTOR=1`. Local runs stay advisory unless those flags are set. Demo-profile doctor still treats Docker as warn-only (`ok` does not include `containerSmoke`).
