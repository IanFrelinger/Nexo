...
public sealed class GenericCommandOrchestrator : IOrchestrator
{
    ...
    private readonly Ashlar.Core.Application.Common.Ports.ILoopKernel _loops;
    ...
    public GenericCommandOrchestrator(IEnumerable<IPreValidator> preValidators,
                                      ...
                                      Ashlar.Core.Application.Common.Ports.ILoopKernel loops)
    {
        _pre = preValidators;
        ...
        ICommand<TIn, TOut> command, TIn input, CancellationToken ct)
    {
        ...
        await _loops.ForEachAsync(_pre, async (v, _, token) =>
        {
            ...
            if (!ok)
            {
                ...
                return Ashlar.Core.Application.Common.Ports.LoopAction.Break;
            }
            return Ashlar.Core.Application.Common.Ports.LoopAction.Continue;
        }, new Ashlar.Core.Application.Common.Ports.LoopOptions { Name = "pre-validators" }, ct);
        ...
        await _loops.ForEachAsync(postValidators, async (v, _, token) =>
        {
            ...
            return Ashlar.Core.Application.Common.Ports.LoopAction.Continue;
        }, new Ashlar.Core.Application.Common.Ports.LoopOptions { Name = "post-validators" }, ct);
