...
/// <c>AddAshlarAutonomy</c> because the loop needs both the objective store (this assembly)
/// and the iteration harness (Infrastructure), and Infrastructure cannot see this layer.
///
/// <para>Separate opt-in from <c>AddAshlarAutonomy</c>: registering the loop's machinery and
...
{
    /// <summary>
    /// Adds the standing loop. Call after <c>AddAshlarAutonomy(configuration)</c>, which supplies
    /// the harness AND the <c>Ashlar:Autonomy</c> options the loop obeys: it runs only under
    ...
    public static IServiceCollection AddAshlarAutonomyLoop(
