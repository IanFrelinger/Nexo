...
/// <summary>
/// Maps Ashlar tool ids to MCP-facing tool names.
///
/// Ashlar ids are dotted (<c>repo.fs.read</c>) but several MCP clients — including the Anthropic
...
/// sanitized name in <see cref="AshlarMcpToolDescriptor"/> for invocation and policy checks.
/// </summary>
public static class McpToolNaming
{
    ...
    /// <summary>
    /// Sanitizes a Ashlar tool id into an MCP-safe name: every character outside
    /// <c>[A-Za-z0-9_-]</c> becomes '_'; the result is trimmed to <see cref="MaxNameLength"/>.
    /// The mapping is deterministic but not injective — <see cref="AshlarMcpToolBridge"/> rejects
