...
public sealed class ChangeProposalStore : IChangeProposalStore
{
    ...
    {
        var fromEnv = Environment.GetEnvironmentVariable("NEXO_FORGE_ROOT");
        if (!string.IsNullOrWhiteSpace(fromEnv))
            return Path.GetFullPath(fromEnv);
        return Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), ".nexo", "runtime-studio", "forge"));
    }
