...
/// NEXO_OLLAMA_* env -> Nexo:Meai config -> legacy OLLAMA_* env -> defaults.
...
public sealed class OllamaEndpointResolverTests
{
    ...
    public void Nexo_env_wins_over_config_and_legacy_env()
    {
        WithCleanEnv(() =>
        {
            Environment.SetEnvironmentVariable(MeaiPipelineOptions.OllamaBaseUrlEnvVar, "http://nexo-env:11434/");
            Environment.SetEnvironmentVariable(MeaiPipelineOptions.OllamaModelEnvVar, " nexo-env-model ");
            ...
            OllamaEndpointResolver.ResolveBaseUrl(options).Should().Be("http://nexo-env:11434");
            OllamaEndpointResolver.ResolveModel(options).Should().Be("nexo-env-model");
        });
    }
    ...
    [Fact]
    public void Bound_Nexo_Meai_section_wins_over_legacy_env()
    {
        WithCleanEnv(() =>
        {
            Environment.SetEnvironmentVariable(MeaiPipelineOptions.LegacyOllamaBaseUrlEnvVar, "http://legacy:11434");
