...
/// <summary>
/// Opt-in structured (JSON lines) console logging shared by the shipped hosts (Ashlar.API and
/// <c>ashlar background-agent daemon</c>). Default output stays the human-readable console formatter;
/// set <c>Ashlar:Logging:Json=true</c> (env <c>Ashlar__Logging__Json=true</c>) or <c>ASHLAR_LOG_JSON=1</c>
/// to switch the console provider to <see cref="ConsoleLoggerExtensions.AddJsonConsole(ILoggingBuilder)"/>.
/// </summary>
public static class AshlarConsoleLoggingExtensions
{
    /// <summary>Configuration key that turns on JSON console output (<c>true</c>/<c>1</c>).</summary>
    public const string JsonConsoleConfigurationKey = "Ashlar:Logging:Json";

    /// <summary>Environment-variable shorthand for <see cref="JsonConsoleConfigurationKey"/> (<c>1</c>/<c>true</c>).</summary>
    public const string JsonConsoleEnvironmentVariable = "ASHLAR_LOG_JSON";
    ...
    public static ILoggingBuilder AddAshlarJsonConsoleIfRequested(this ILoggingBuilder logging, IConfiguration configuration)
    {
