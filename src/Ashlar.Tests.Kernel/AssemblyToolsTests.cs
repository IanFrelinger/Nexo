...
{
    // Any real, built Ashlar assembly works here — these tests exercise the assembly
    // TOOLS (analyze/decompile/security-scan), not this particular library. It used
    // to point at Ashlar.Core.dll; that project was retired, so it now uses
    // Ashlar.Core.Domain.dll, which the test project already copies to its output.
    private static string SampleAssemblyPath =>
        Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "Ashlar.Core.Domain.dll"));
    ...
    public async Task AssemblyAnalyzeTool_reads_metadata_from_built_assembly()
    {
        ...
        result.Delta.Log.Should().ContainSingle().Which.Should().Contain("Ashlar.Core");
    ...
    public async Task AssemblyDecompileTool_returns_error_when_file_missing()
    {
        ...
        var outDir = Path.Combine(Path.GetTempPath(), "ashlar-decompile-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var badDll = Path.Combine(Path.GetTempPath(), "ashlar-bad-" + Guid.NewGuid() + ".dll");
    ...
    {
        var badDll = Path.Combine(Path.GetTempPath(), "ashlar-scan-" + Guid.NewGuid() + ".dll");
        await File.WriteAllTextAsync(badDll, "not a real assembly");
        try
        {
            var tool = new AssemblySecurityScanTool();
