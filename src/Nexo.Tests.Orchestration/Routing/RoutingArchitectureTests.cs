...
{
    [Fact]
    public void NexoOrchestrationRouting_MustNotReference_NexoRuntime()
    {
        ...
        references.Should().NotContain(x => x!.Contains("Nexo.Runtime", StringComparison.OrdinalIgnoreCase));
    }

    [Fact]
    public void NexoAbstractionsBarriers_MustOnlyUse_System_And_BarriersNamespaces()
    {
        var assembly = typeof(BarrierContext).Assembly;
        var barrierTypes = assembly.GetTypes()
            .Where(t => t.Namespace == "Nexo.Abstractions.Barriers")
            .ToList();
        ...
        foreach (var type in barrierTypes)
        {
            foreach (var referencedType in GetReferencedTypes(type))
            {
                ...
                if (referencedType.Namespace.StartsWith("System", StringComparison.Ordinal))
                    continue;
                if (referencedType.Namespace.StartsWith("Nexo.Abstractions.Barriers", StringComparison.Ordinal))
                    continue;
