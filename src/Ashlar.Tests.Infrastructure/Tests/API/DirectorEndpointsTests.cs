...
public sealed class DirectorEndpointsTests
{
    ...
    public async Task RunDirectorWorkflow_WhenOrchestratorFails_ReturnsDailyAndPersistsEntry()
    {
        ...
        try
        {
            var configuration = new ConfigurationBuilder()
                ...
                {
                    ["ASHLAR_DAILIES_PATH"] = dailiesPath
                })
                .Build();
    ...
    {
        var method = typeof(AshlarEndpoints).GetMethod(name, BindingFlags.NonPublic | BindingFlags.Static);
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"ashlar-director-endpoint-tests-{Guid.NewGuid():N}");
