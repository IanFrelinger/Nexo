...
public sealed class AnalyzerFenceGateTests
{
    ...
    private const string DirtyBrickSource = """
        ...
        namespace Ashlar.Certified.AnalyzerProbe;
    ...
    public async Task Wall_clock_candidate_fails_with_a_located_verbatim_finding()
    {
        ...
        finding.Id.Should().Be("ASHLAR0006");
        ...
        feedback.Should().Contain("ASHLAR0006").And.Contain(finding.Message).And.Contain($"line {finding.Line}");
    }
    ...
    public async Task Missing_brick_anchor_references_fail_closed()
    {
        ...
        outcome.Passed.Should().BeFalse();
        outcome.FailureReason.Should().Contain("Ashlar.Core.Domain.Bricks.Brick");
    }
    ...
    public async Task Manifest_rules_run_alongside_the_catalog_and_restate_the_instruction_verbatim()
    {
        ...
        outcome.Findings.Should().Contain(f => f.Id == "ASHLAR0006", "the static catalog still runs");
    ...
    public async Task Wrapper_injected_text_is_exempt_from_manifest_rules()
    {
        ...
        outcome.Passed.Should().BeFalse();
        outcome.Findings.Should().OnlyContain(f => f.Id == "ASHLAR0006",
    ...
    {
        var touchSet = new Ashlar.Core.Application.Autonomy.TouchSet
        {
    ...
    public async Task UndeclaredKernelReference_FailsTheGate_WithTheSmugglingDiagnostic()
    {
        ...
        var attack = MutationProbeBrickSource.Code.Replace(
            ...
            "var errorCount = errorLines.Count + "
            + "(Ashlar.Core.Application.Autonomy.TrustKernel.KernelPathPrefixes.Count * 0);");

        var outcome = await Gate().EvaluateAsync(
            attack,
            ContractReferences.Append(typeof(Ashlar.Core.Application.Autonomy.TrustKernel).Assembly.Location),
            touchSet: touchSet);

        outcome.Passed.Should().BeFalse();
        var finding = outcome.Findings.First(
            f => f.Id == Ashlar.Analyzers.TouchSetReferenceAnalyzer.UndeclaredKernelReferenceId);
