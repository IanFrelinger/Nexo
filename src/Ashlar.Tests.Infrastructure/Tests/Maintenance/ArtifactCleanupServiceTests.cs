...
public class ArtifactCleanupServiceTests
{
    ...
    {
        var repoRoot = Path.Combine(Path.GetTempPath(), $"ashlar_service_test_{Guid.NewGuid():N}");
    ...
    {
        var repoRoot = Path.Combine(Path.GetTempPath(), $"ashlar_service_specific_{Guid.NewGuid():N}");
        Directory.CreateDirectory(repoRoot);
        try
        {
