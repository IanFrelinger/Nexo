...
public class InfrastructureGapCoverageTests
{
    ...
    [Fact]
    public void LocalAshlarInstanceCapabilitiesProvider_returns_local()
    {
        new LocalAshlarInstanceCapabilitiesProvider().GetCapabilities().Should().Be(InstanceCapabilities.LocalAshlar);
    }
    ...
    public async Task ConflictResolver_picks_first_candidate_or_null()
    {
        ...
        var manifest = new BrickManifest
        {
            Id = "brick-1",
            Name = "n",
            Interface = new Ashlar.Core.Domain.Bricks.BrickInterface(),
        };
    ...
    {
        var model = new Mock<Ashlar.Abstractions.IModel>();
    ...
    {
        var model = new Mock<Ashlar.Abstractions.IModel>();
        model.Setup(m => m.CompleteAsync(It.IsAny<Ashlar.Abstractions.ModelInput>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new Ashlar.Abstractions.ModelOutput("<osm version=\"0.6\"></osm>"));
    ...
    public void BrickHostServiceCollectionExtensions_binds_options()
    {
        ...
        services.AddAshlarBrickHostOptions(o => o.RemoteCatalogBaseUrls = new[] { "http://remote" });
