...
using Ashlar.Infrastructure.Export;
using Xunit;
...
public class InfrastructureExecutionGapCoverageTests
{
    ...
    public async Task RemoteBrick_executes_via_http_and_maps_response()
    {
        ...
        var entry = new BrickCatalogEntryDto
        {
            ...
            HostCapabilities = new Ashlar.Brick.Contracts.Capabilities.NodeCapabilityManifestDto
            {
                NodeId = "node-1",
                SupportedCapabilities = new[] { Ashlar.Brick.Contracts.Capabilities.TaskCapabilityDto.CodeGeneration },
            },
        };
    ...
    {
        var result = new Ashlar.Infrastructure.Execution.BehaviorResult
        {
            Success = false,
    ...
    {
        var key = "ASHLAR_LOAD_PREFERENCE";
