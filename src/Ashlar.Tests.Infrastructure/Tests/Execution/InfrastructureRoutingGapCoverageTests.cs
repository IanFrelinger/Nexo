...
using Ashlar.Infrastructure.Execution.Sdk.Extensions;
using Xunit;
...
public class InfrastructureRoutingGapCoverageTests
{
    ...
    [Fact]
    public void AddAshlarFederatedBrickMesh_throws_without_local_registry()
    {
        ...
        var act = () => services.AddAshlarFederatedBrickMesh(config).BuildServiceProvider().GetRequiredService<IBrickRegistry>();
        act.Should().Throw<InvalidOperationException>().WithMessage("*BrickRegistry*");
    }

    [Fact]
    public void AddAshlarFederatedBrickMesh_returns_local_registry_without_remote_urls()
    {
        var services = new ServiceCollection();
        services.AddHttpClient();
        ...
        services.AddAshlarFederatedBrickMesh(config);
    ...
    [Fact]
    public void AddAshlarFederatedBrickMesh_rejects_invalid_remote_url()
    {
        var services = new ServiceCollection();
        services.AddHttpClient();
        services.AddSingleton(new BrickRegistry(Array.Empty<DomainBrick>()));
    ...
    [Fact]
    public void AddAshlarFederatedBrickMesh_builds_composite_registry_for_valid_remote_urls()
    {
        var services = new ServiceCollection();
        services.AddHttpClient();
        services.AddSingleton(new BrickRegistry(Array.Empty<DomainBrick>()));
