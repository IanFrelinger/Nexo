...
public abstract class MultiPlatformTestBase : TestBase
{
    ...
    protected string DiscoverProjectRoot()
    {
        ...
        while (dir != null)
        {
            if (File.Exists(Path.Combine(dir.FullName, "application", "src", "Ashlar.CLI", "Ashlar.CLI.csproj")))
            {
                return dir.FullName;
            }
            dir = dir.Parent;
    ...
    protected async Task<TestResult> ExecuteContainerTestAsync(string dockerfile, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var imageTag = $"ashlar-framework-test:{_platformName}";
            var ephemeral = string.Equals(Environment.GetEnvironmentVariable("ASHLAR_TEST_EPHEMERAL"), "1", StringComparison.OrdinalIgnoreCase);
