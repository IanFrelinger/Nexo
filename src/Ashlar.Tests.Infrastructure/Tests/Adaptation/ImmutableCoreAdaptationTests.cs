...
public sealed class ImmutableCoreAdaptationTests : IDisposable
{
    ...
    {
        (_, _tempDirCleanup) = TestHelpers.CreateTempDirectoryWithCleanup("ashlar-immutable-adaptation");
    }
    ...
    {
        var registry = new ImmutableCoreRegistry();
        var path = "src/Ashlar.Infrastructure/Observation/PatternDetector.cs";
    ...
    {
        var registry = new ImmutableCoreRegistry();
        var path = "src/Ashlar.Infrastructure/Analysis/Rules/SecurityAnalysisRule.cs";
    ...
    {
        var registry = new ImmutableCoreRegistry();
        var path = "src/Ashlar.Infrastructure/Validation/Parsers/TrxTestResultParser.cs";
    ...
    {
        var registry = new ImmutableCoreRegistry();
        var path = "src/Ashlar.Infrastructure/Trust/AccessBoundary.cs";
    ...
    {
        var registry = new ImmutableCoreRegistry();
        var path = "src/Ashlar.Infrastructure/Rollback/RollbackManager.cs";
    ...
    {
        var storePath = Path.Combine(Path.GetTempPath(), $"ashlar-immutable-audit-{Guid.NewGuid():N}.db");
    ...
    {
        var storePath = Path.Combine(Path.GetTempPath(), $"ashlar-immutable-immediate-{Guid.NewGuid():N}.db");
        var failures = new List<TestFailureRecord>
        {
            new()
            {
                ...
                Timestamp = DateTimeOffset.UtcNow,
                TestName = "SomeTest",
                FilePath = "src/Ashlar.Infrastructure/Observation/PatternDetector.cs",
                ErrorMessage = "Test failed",
            },
        };
