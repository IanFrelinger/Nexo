...
using Xunit;
using OutputFormat = Ashlar.Core.Domain.Export.OutputFormat;

namespace Ashlar.Tests.Infrastructure.Tests.Export;
...
public class InfrastructureExportGapCoverageTests
{
    ...
    public async Task WorkflowExporter_runtime_mode_includes_providers_and_bootstrap()
    {
        ...
        result.Success.Should().BeTrue();
        result.RuntimeRequirements.Should().Contain("Ashlar.Runtime");
        result.RuntimeRequirements.Should().Contain("Ashlar.Provider.openai");
