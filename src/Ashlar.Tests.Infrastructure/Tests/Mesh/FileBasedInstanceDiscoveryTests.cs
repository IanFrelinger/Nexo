...
public sealed class FileBasedInstanceDiscoveryTests : IDisposable
{
    ...
    {
        _path = Path.Combine(Path.GetTempPath(), $"ashlar-instances-{Guid.NewGuid():N}.json");
        _prevMeshPolicy = Environment.GetEnvironmentVariable("ASHLAR_MESH_TRUST_POLICY");
        _prevInstancesPath = Environment.GetEnvironmentVariable("ASHLAR_MESH_INSTANCES_PATH");
    }

    public void Dispose()
    {
        /// <summary>Restore env.</summary>
        RestoreEnv("ASHLAR_MESH_TRUST_POLICY", _prevMeshPolicy);
        /// <summary>Restore env.</summary>
        RestoreEnv("ASHLAR_MESH_INSTANCES_PATH", _prevInstancesPath);
        try
        {
    ...
    public async Task Allowlist_policy_excludes_non_admitted_peers()
    {
        ...
        Environment.SetEnvironmentVariable("ASHLAR_MESH_INSTANCES_PATH", _path);
        Environment.SetEnvironmentVariable("ASHLAR_MESH_TRUST_POLICY", "allowlist");
