...
/// Enable with <c>ASHLAR_RUN_MESH_LAB=1</c> (requires Docker, bash, python3).
/// </summary>
public sealed class MeshLabDockerFixture : IAsyncLifetime
{
    ...
    private static readonly Mutex s_meshLabGate =
        new(initiallyOwned: false, name: "Ashlar.MeshLab.DockerE2E.v1");
    ...
    public async Task InitializeAsync()
    {
        ...
        {
            _skipReason = "Set ASHLAR_RUN_MESH_LAB=1 with Docker, bash, and python3 available.";
            return;
        }

        if (!s_meshLabGate.WaitOne(TimeSpan.FromMinutes(45)))
        {
            _skipReason = "Timed out waiting for mesh lab gate (another ASHLAR_RUN_MESH_LAB test may be running).";
            return;
        }
        ...
        _envFilePath = Path.Combine(Path.GetTempPath(), $"ashlar-mesh-lab-{Guid.NewGuid():N}.env");
        await File.WriteAllTextAsync(
                _envFilePath,
                $"""
                 Ashlar__Security__ApiKey={key}
                 Ashlar__Security__PeerB__BearerToken={bearer}
                 Ashlar__Security__Worker__BasicAuthPassword={basic}
        ...
        _composeProjectName = $"ashlar_mesh_dotnet_{Guid.NewGuid():N}"[..32];
