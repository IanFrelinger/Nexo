...
public sealed class OnboardingStatusMeaiOllamaProdStyleTests
{
    ...
    {
        // The compose stacks set Ashlar__Meai__OllamaBaseUrl/OllamaModel so a container dials the
        ...
        // because AddAshlar(options => ...) builds its own IConfiguration from environment
        // variables only (AshlarServiceCollectionExtensions.AddAshlar) - host configuration such as
        // appsettings.json never reaches Ashlar:Meai. That gap is a separate finding; this test
        // pins the endpoint's contract, which is "report the bound options".
        using var factory = CreateFactory(new Dictionary<string, string?>
        {
            ["Ashlar:Meai:OllamaBaseUrl"] = "http://ollama:11434",
            ["Ashlar:Meai:OllamaModel"] = "qwen2.5-coder:7b",
