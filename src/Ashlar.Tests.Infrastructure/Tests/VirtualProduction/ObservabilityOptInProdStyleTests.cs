...
/// <summary>
/// ProdStyle coverage for the opt-in observability switches on the real Ashlar.API host:
/// <list type="bullet">
...
///   <item><c>ASHLAR_LOG_JSON=1</c> / <c>Ashlar:Logging:Json=true</c>: the console provider switches to the JSON
...
public sealed class ObservabilityOptInProdStyleTests
{
    ...
    public async Task Otlp_endpoint_on_an_unreachable_port_enables_export_without_failing_startup()
    {
        ...
        factory.Services.GetRequiredService<IMetricsCollector>().Should().BeOfType<OpenTelemetryMetricsCollector>(
            "AddAshlarOpenTelemetry replaces the in-process collector once OTLP export is on");
