...
/// <summary>
/// ProdStyle coverage for the MCP + A2A protocol surfaces on the real Ashlar.API pipeline:
...
public sealed class McpA2AProtocolIngressProdStyleTests
{
    ...
    private sealed class TestAgentCatalog : IAshlarA2AAgentCatalog
    {
        public IReadOnlyList<AshlarA2AAgentDescriptor> GetAgents() =>
        [
            new AshlarA2AAgentDescriptor(
                Id: "echo-agent",
                Name: "Echo Agent",
    ...
    {
        var factory = new AshlarApiWebApplicationFactory().WithWebHostBuilder(builder =>
        {
            ...
            if (withTestAgent)
            {
                builder.ConfigureServices(services =>
                {
                    ...
                    services.AddSingleton<IAshlarA2AAgentCatalog, TestAgentCatalog>();
    ...
    public async Task Mcp_requires_credentials_on_every_verb_when_auth_is_configured()
    {
        using var scope = CreateFactory(ApiKeyAuthSettings(
            ...
            ("Ashlar:Mcp:Server:ExposedToolIds:0", "repo.fs.read")));
        using var client = scope.CreateClient();
    ...
    public async Task Mcp_initialize_succeeds_with_credentials_and_advertises_the_server()
    {
        ...
        body.Should().Contain("\"protocolVersion\"").And.Contain("ashlar");
    }
    ...
    public async Task A2a_agent_card_is_credentialed_by_default_and_anonymous_only_by_opt_in()
    {
        var settings = ApiKeyAuthSettings(
            ("Ashlar:A2A:Server:Enabled", "true"),
            ("Ashlar:A2A:Server:PublicBaseUrl", "http://localhost"),
            ("Ashlar:A2A:Server:ExposedAgentIds:0", "echo-agent"));
        ...
        using (var client = scope.CreateClient())
        {
            ...
            withKey.Headers.Add("X-Ashlar-Api-Key", ApiKey);
        ...
        settings["Ashlar:A2A:Server:AllowAnonymousAgentCard"] = "true";
        using (var scope = CreateFactory(settings, withTestAgent: true))
        using (var client = scope.CreateClient())
        {
            var anonymous = await client.GetAsync("/.well-known/agent-card.json");
    ...
    public async Task A2a_message_send_round_trips_to_the_agent_through_the_real_pipeline()
    {
        using var scope = CreateFactory(
            ...
            withTestAgent: true);
        using var client = scope.CreateClient();
        client.DefaultRequestHeaders.Add("X-Ashlar-Api-Key", ApiKey);

        // The real A2A SDK client against the real Ashlar.API pipeline — auth middleware,
    ...
    // The air-gapped enable refusal sets the process-wide ASHLAR_DEPLOYMENT_PROFILE and therefore
