...
/// Ashlar.API pipeline. <c>MapFallbackToFile("index.html")</c> serves the SPA for unmatched paths so
...
/// remote-execution routes are "mapped only when Ashlar:Execution:ServeRemoteExecution=true (404
...
{
    private static WebApplicationFactory<Program> CreateFactory() => new AshlarApiWebApplicationFactory();

    [Theory(Timeout = TestTimeouts.HostTouching)]
    [InlineData("/api/definitely-not-a-route")]
    [InlineData("/api/execution/build")]   // unmapped unless Ashlar:Execution:ServeRemoteExecution=true
