...
/// the k8s sample rely on, on the real Ashlar.API pipeline. <c>/health</c> is liveness (constant 200 while
...
public sealed class HealthAndReadinessProdStyleTests
{
    ...
    public async Task Ready_is_503_while_starting_200_once_started_and_503_again_once_stopping()
    {
        ...
        AshlarEndpoints.EvaluateReadiness(started.Token, stopping.Token)
    ...
    public async Task Probes_are_reachable_without_credentials_when_built_in_auth_is_on()
    {
        using var factory = CreateFactory(new Dictionary<string, string?>
        {
            ...
            ["Ashlar:Security:ApiKey"] = "prodstyle-probe-key",
            ["Ashlar:Security:AuthorizationScope"] = "AllApi",
        });
        using var client = factory.CreateClient();
