...
namespace Ashlar.Tests.Infrastructure.Tests.Architecture;
...
public sealed class DependencyWrappingArchitectureTests : UnitTestBase
{
    public override Task<TestResult> ExecuteAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            // Scan all Ashlar assemblies in the current test output directory (excluding test assemblies)
    ...
    {
        var dlls = Directory.GetFiles(baseDirectory, "Ashlar*.dll", SearchOption.TopDirectoryOnly)
            .Where(p =>
                !Path.GetFileName(p).StartsWith("Ashlar.Tests.", StringComparison.OrdinalIgnoreCase)
