...
/// <summary>
/// Block 4 dogfood gate: promote one Ashlar fix via inheritance.
/// Validates IAdaptationPromoter records a promoted fix for a Ashlar brick.
/// </summary>
...
public sealed class DogfoodBlock4Tests : IDisposable
{
    ...
    {
        (_tempDir, _tempDirCleanup) = TestHelpers.CreateTempDirectoryWithCleanup("ashlar-dogfood-block4");
    }

    public void Dispose() => _tempDirCleanup.Dispose();

    [Fact(Timeout = 15000)]
    public async Task PromoteAshlarFix_ViaInheritance_RecordStoredWithPromotedTrue()
    {
        ...
        var record = new AdaptationRecord
        {
            ...
            Promoted = false,
            Message = "Dogfood: promoted Ashlar fix via inheritance",
        };
