...
/// proposals, mode, and a short <c>daemon</c> run. Spawns the real <c>Ashlar.CLI.dll</c> via <see cref="CliRunner"/>
...
public sealed class RuntimeStudioBlackBoxSmokeTests : E2ETestBase
{
    ...
    public RuntimeStudioBlackBoxSmokeTests() : base("ashlar-runtime-studio-blackbox") { }
    ...
    private static IReadOnlyDictionary<string, string?> StudioEnv(string tempRoot)
    {
        ...
        {
            ["ASHLAR_OBSERVATIONS_PATH"] = Path.Combine(tempRoot, "observations.jsonl"),
            ["ASHLAR_OBJECTIVES_ROOT"] = Path.Combine(tempRoot, "objectives"),
            ["ASHLAR_FORGE_ROOT"] = Path.Combine(tempRoot, "forge"),
            ["ASHLAR_AGENT_MODE_PATH"] = Path.Combine(tempRoot, "agent-mode.json"),
        };
    }
    ...
    public async Task Daemon_minimal_config_timed_run_exits_zero()
    {
        ...
        var merged = new Dictionary<string, string?>(env) { ["ASHLAR_ALLOW_MOCK"] = "1" };
    ...
    public async Task Proposals_disk_seed_lists_and_shows()
    {
        ...
        var forgeRoot = env["ASHLAR_FORGE_ROOT"]!;
