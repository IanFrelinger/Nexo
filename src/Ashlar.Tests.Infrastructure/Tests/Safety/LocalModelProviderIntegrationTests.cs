...
public sealed class LocalModelProviderIntegrationTests
{
    ...
    private sealed class LocalModelTestScope : IAsyncDisposable
    {
            ...
            try
            {
                ...
                if (modelPath is null)
                {
                    return new LocalModelTestScope(
                        ...
                        previousContext,
                        isReady: false,
                        skipReason: "No GGUF model available. Set ASHLAR_LOCAL_MODEL_PATH or allow fixture download.");
                }

                Environment.SetEnvironmentVariable("ASHLAR_LOCAL_MODEL_PATH", modelPath);
                Environment.SetEnvironmentVariable("ASHLAR_LOCAL_CONTEXT_SIZE", contextSize);
        ...
        public ValueTask DisposeAsync()
        {
            /// <summary>Reset loaded model state.</summary>
            ResetLoadedModelState();
            Environment.SetEnvironmentVariable("ASHLAR_LOCAL_MODEL_PATH", _previousPath);
            Environment.SetEnvironmentVariable("ASHLAR_LOCAL_CONTEXT_SIZE", _previousContext);
            return ValueTask.CompletedTask;
        }

        private static async Task<string?> ResolveModelPathAsync()
        {
            var fromEnv = Environment.GetEnvironmentVariable("ASHLAR_LOCAL_MODEL_PATH")?.Trim();
            if (!string.IsNullOrWhiteSpace(fromEnv) && File.Exists(fromEnv))
                return fromEnv;

            var cacheDir = Path.Combine(Path.GetTempPath(), "ashlar-gguf-cache");
