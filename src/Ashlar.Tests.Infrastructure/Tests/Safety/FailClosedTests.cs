...
public sealed class FailClosedTests : IDisposable
{
    ...
    {
        (_, _tempDirCleanup) = TestHelpers.CreateTempDirectoryWithCleanup("ashlar-fail-closed");
    }
    ...
    public async Task ImmutableCoreRegistry_WhenStorageUnavailable_RejectsAllAdaptations()
    {
        ...
        var storePath = Path.Combine(Path.GetTempPath(), $"ashlar-failclosed-{Guid.NewGuid():N}");
    ...
    {
        var (tempDir, _) = TestHelpers.CreateTempDirectoryWithCleanup("ashlar-rollback-fail");
    ...
    public async Task SelfImprovementLoop_WhenAccessBoundaryPaused_SkipsAdaptation()
    {
        ...
        var storePath = Path.Combine(Path.GetTempPath(), $"ashlar-failclosed-pause-{Guid.NewGuid():N}");
        var services = new ServiceCollection()
            .AddLogging(b => b.AddConsole().SetMinimumLevel(LogLevel.Warning))
            .AddSingleton<ITestFailureStore>(new MockTestFailureStore(failures))
            .AddCodeAnalyzers()
