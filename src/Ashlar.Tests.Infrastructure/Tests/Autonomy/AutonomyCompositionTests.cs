...
public sealed class AutonomyCompositionTests
{
    ...
    public void OptionsAreFailClosed_AndDisabledIsAlwaysValid()
    {
        ...
        var options = provider.GetRequiredService<IOptions<AshlarAutonomyOptions>>().Value;

        options.Enabled.Should().BeFalse("a host that merely composes the loop must not run it");
        new ValidateAshlarAutonomyOptions().Validate(null, options).Succeeded.Should().BeTrue();
    }
    ...
    {
        var options = new AshlarAutonomyOptions
        {
            Enabled = true,
        ...
        var result = new ValidateAshlarAutonomyOptions().Validate(null, options);
    ...
    [Fact]
    public void ConfigurationBinds_FromTheAshlarAutonomySection()
    {
        var services = new ServiceCollection();
        services.AddLogging();
        ...
        services.AddAshlarAutonomy(new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["Ashlar:Autonomy:SessionImage"] = "proposer:latest",
                ["Ashlar:Autonomy:SessionImageDigest"] = "sha256:0123abcd",
                ["Ashlar:Autonomy:CadenceFloorSeconds"] = "42",
            })
            .Build());
    ...
    {
        // The three session flags default to false so a bare AddAshlarAutonomy() stays valid,
        ...
        services.AddAshlarAutonomy(new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["Ashlar:Autonomy:UseSandboxSessions"] = "true",
                ["Ashlar:Autonomy:BuildCandidateInSession"] = "true",
                ["Ashlar:Autonomy:ExecuteCandidateInSession"] = executeInSession ? "true" : "false",
                ["Ashlar:Autonomy:SessionImage"] = "proposer:latest",
            })
            .Build());
        using var provider = services.BuildServiceProvider();
    ...
    public async Task ConfiguredDigestPin_ReachesTheComposedSessionRunner_AndRefusesAMismatch()
    {
        ...
        services.AddAshlarAutonomy(new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["Ashlar:Autonomy:UseSandboxSessions"] = "true",
                ["Ashlar:Autonomy:SessionImage"] = "proposer:latest",
                ["Ashlar:Autonomy:SessionImageDigest"] = "sha256:pinned",
            })
            .Build());
        using var provider = services.BuildServiceProvider();
...
/// ASHLAR_DEPLOYMENT_PROFILE=airgapped leaking into a concurrently running host-composition test
