...
/// whichever order a host calls <c>AddAshlarAutonomy</c> and <c>AddAshlarAutonomyDigestJob</c>,
...
public sealed class AutonomyDigestJobTests
{
    ...
    public void DigestJob_TakesOverTheSinkBinding_InEitherCompositionOrder(bool digestFirst)
    {
        ...
        if (digestFirst)
        {
            services.AddAshlarAutonomyDigestJob();
    ...
    private static AutonomyDigestService Service(
        ...
        sink,
        Options.Create(new AshlarAutonomyOptions { DigestIntervalSeconds = digestIntervalSeconds }),
