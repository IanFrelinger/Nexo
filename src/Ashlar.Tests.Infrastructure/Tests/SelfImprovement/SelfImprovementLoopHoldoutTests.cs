...
/// Full holdout validation E2E is covered by ashlar improve --self with --holdout-filter in dogfood.
/// </summary>
...
public sealed class SelfImprovementLoopHoldoutTests : IDisposable
{
    ...
    {
        (_, _tempDirCleanup) = TestHelpers.CreateTempDirectoryWithCleanup("ashlar-holdout");
    }
    ...
    {
        var metricsPath = Path.Combine(Path.GetTempPath(), $"ashlar-metrics-{Guid.NewGuid():N}.json");
