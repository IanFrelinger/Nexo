...
public sealed class HelloGen04480915CommandTests : UnitTestBase
{
    public override async Task<TestResult> ExecuteAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var (code, stdout, stderr) = await RunDotnetAsync(
                repoRoot,
                "run --project src/Nexo.CLI -- demo hello-gen-04480915 --format-json",
                cancellationToken);
            ...
            AssertEqual("hello-gen-04480915", doc.RootElement.GetProperty("command").GetString());
            AssertEqual("hello from generated command", doc.RootElement.GetProperty("message").GetString());

            return new TestResult
            {
                Name = nameof(HelloGen04480915CommandTests),
