...
public static class RepoPathResolver
{
    ...
    /// </summary>
    public const string StateDirectoryEnvironmentVariable = "NEXO_STATE_DIR";

    /// <summary>
    /// Default runtime-state directory relative to the repo/app root (<c>.nexo/</c> is gitignored).
    /// </summary>
    public const string DefaultStateDirectoryRelativePath = ".nexo/state";
    ...
    /// <summary>
    /// Resolves the directory that holds Nexo runtime state (LiteDB stores such as
    /// <c>nexo-patterns.db</c>, <c>nexo-adaptation.db</c>, <c>nexo-copilot-tasks.db</c>, and
    /// <c>nexo-snapshots/</c>). Reads <see cref="StateDirectoryEnvironmentVariable"/> and defers to
    ...
    /// <paramref name="repoRoot"/> and no <c>.nexo/state/</c> yet — kept so existing installs keep
    /// reading their data; move the <c>nexo-*.db</c> files and <c>nexo-snapshots/</c> into
    /// <c>.nexo/state/</c> to migrate) → <c>&lt;repoRoot&gt;/.nexo/state</c>.
    /// </summary>
    /// <param name="repoRoot">Repository / application root the default and relative overrides hang off.</param>
    /// <param name="configuredStateDirectory">Override (normally the <c>NEXO_STATE_DIR</c> value); null or blank = none.</param>
    /// <returns>Absolute state directory path. Created when it does not exist yet (best effort).</returns>
    ...
    /// <summary>
    /// True when pre-<c>.nexo/state</c> LiteDB state files (<c>nexo-*.db</c>) live directly at <paramref name="root"/>.
    /// </summary>
    private static bool HasLegacyStateFiles(string root)
    {
        try
        {
            return Directory.Exists(root) && Directory.EnumerateFiles(root, "nexo-*.db").Any();
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            return false;
