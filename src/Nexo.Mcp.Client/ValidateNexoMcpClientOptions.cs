...
/// <summary>
/// Fail-closed startup validation for <see cref="NexoMcpClientOptions"/> (wired via
...
/// </summary>
public sealed class ValidateNexoMcpClientOptions : IValidateOptions<NexoMcpClientOptions>
{
    ...
    /// <inheritdoc />
    public ValidateOptionsResult Validate(string? name, NexoMcpClientOptions options)
    {
        if (!options.Enabled)
        {
        ...
        {
            failures.Add(
                $"{nameof(NexoMcpClientOptions.Enabled)}=true is not permitted under the AirGapped deployment profile " +
                $"({DeploymentProfileVariable}={profile}). The MCP client dials external servers and stays off.");
        }

        if (options.ConnectTimeout <= TimeSpan.Zero)
        {
            failures.Add($"{nameof(NexoMcpClientOptions.ConnectTimeout)} must be positive (got {options.ConnectTimeout}).");
        }

        if (options.ToolListRefreshInterval < TimeSpan.Zero)
        {
            failures.Add($"{nameof(NexoMcpClientOptions.ToolListRefreshInterval)} must be zero (disabled) or positive.");
        }
