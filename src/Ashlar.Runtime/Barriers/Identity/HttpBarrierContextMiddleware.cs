...
namespace Ashlar.Runtime.Barriers.Identity;
...
/// and records an audit event. Gated by ASHLAR_BARRIER_MIDDLEWARE_ENABLED.
/// </summary>
public sealed class HttpBarrierContextMiddleware : IMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        ...
        try
        {
            var enabled = Environment.GetEnvironmentVariable("ASHLAR_BARRIER_MIDDLEWARE_ENABLED");
    ...
    private static BarrierResolutionContext BuildResolutionContext(HttpContext context, string correlationId)
    {
        ...
        headers.TryGetValue("x-ashlar-barrier", out var explicitLevel);
        headers.TryGetValue("x-ashlar-api-key", out var apiKey);
