...
/// It injects ashlar.model.prefer/provider directives based on the current runtime spec (default scope).
/// </summary>
public sealed class OrchestrationRuntimeModelDecorator : IModel
{
    ...
    /// <summary>Injects ashlar.model preference directives into the first system message.</summary>
    public static ModelInput InjectDirectives(ModelInput input, string? prefer, string? provider, string? model)
    {
        var directives = new List<string>();
        if (!string.IsNullOrWhiteSpace(prefer)) directives.Add($"ashlar.model.prefer={prefer}");
        if (!string.IsNullOrWhiteSpace(provider)) directives.Add($"ashlar.model.provider={provider}");
        if (!string.IsNullOrWhiteSpace(model)) directives.Add($"ashlar.model.name={model}");
