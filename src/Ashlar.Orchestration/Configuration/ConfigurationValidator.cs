...
public sealed class ConfigurationValidator
{
    ...
    private void ValidateRequiredSettings(List<ConfigurationError> errors)
    {
        ...
        {
            "Ashlar:Orchestration:MaxRetries",
            "Ashlar:Orchestration:DefaultTimeout"
        };
    ...
    private void ValidateNumericRanges(List<ConfigurationError> errors, List<ConfigurationWarning> warnings)
    {
        ...
        var maxRetries = _configuration.GetValue<int?>("Ashlar:Orchestration:MaxRetries");
        if (maxRetries.HasValue)
        {
            if (maxRetries < 1 || maxRetries > 10)
            {
                errors.Add(new ConfigurationError
                {
                    Setting = "Ashlar:Orchestration:MaxRetries",
        ...
        // Validate timeout
        var timeoutSeconds = _configuration.GetValue<int?>("Ashlar:Orchestration:DefaultTimeout");
        if (timeoutSeconds.HasValue)
        {
            if (timeoutSeconds < 1 || timeoutSeconds > 3600)
            {
                errors.Add(new ConfigurationError
                {
                    Setting = "Ashlar:Orchestration:DefaultTimeout",
    ...
    private void ValidateConnectionStrings(List<ConfigurationError> errors, List<ConfigurationWarning> warnings)
    {
        ...
        var useLLM = _configuration.GetValue<bool>("Ashlar:Orchestration:UseLLM", false);
        if (useLLM)
        {
            var apiKey = _configuration["Ashlar:LLM:ApiKey"];
            if (string.IsNullOrWhiteSpace(apiKey))
            {
                warnings.Add(new ConfigurationWarning
                {
                    Setting = "Ashlar:LLM:ApiKey",
