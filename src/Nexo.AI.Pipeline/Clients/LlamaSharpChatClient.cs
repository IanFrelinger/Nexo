...
public sealed class LlamaSharpChatClient : IChatClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path))
        {
            throw new InvalidOperationException(
                "Local model not configured. Set Nexo:Meai:LocalModelPath or NEXO_LOCAL_MODEL_PATH to a GGUF file.");
        }
    ...
    private string? ResolveModelPath()
    {
        ...
        {
            path = Environment.GetEnvironmentVariable("NEXO_LOCAL_MODEL_PATH");
        }
