...
namespace Nexo.AI.Pipeline.Models;
        ...
        foreach (var (role, content) in input.Messages)
        {
            if (string.Equals(role, "system", StringComparison.OrdinalIgnoreCase))
            {
                ...
                foreach (var line in (content ?? string.Empty).Split('\n'))
                {
                    ...
                    if (trimmed.StartsWith("nexo.model.capability=", StringComparison.OrdinalIgnoreCase))
                    {
                        capability = trimmed["nexo.model.capability=".Length..].Trim();
                        continue;
                    }
