...
/// <summary>
/// Options for exposing Nexo agents over the A2A protocol.
///
...
/// a terminal task within the request (Nexo has no durable task machinery yet; task state lives
/// in per-agent in-memory stores good for <c>tasks/get</c> within a process lifetime).
/// </summary>
public sealed class NexoA2AServerOptions
{
    /// <summary>Default configuration section (<c>Nexo:A2A:Server</c>).</summary>
    public const string SectionPath = "Nexo:A2A:Server";

    /// <summary>Master switch; defaults to <see langword="false"/> (no endpoints are mapped).</summary>
    public bool Enabled { get; set; }

    /// <summary>
    /// Explicit allowlist of Nexo agent ids to expose. Empty exposes zero agents unless
    ...
    /// <summary>
    /// Public absolute base URL of this host (e.g. <c>https://nexo.example.com</c>); used to
