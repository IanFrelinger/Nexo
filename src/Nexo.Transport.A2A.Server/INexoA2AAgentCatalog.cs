...
/// <summary>
/// Port supplying the Nexo agents that may be considered for A2A exposure.
///
/// Deliberately DTO-based with no reference to <c>Nexo.Core.Domain</c>: the transport layering
...
/// decisions.
/// </summary>
public interface INexoA2AAgentCatalog
{
    /// <summary>All candidate agents (unfiltered; the adapter applies the exposure policy).</summary>
    IReadOnlyList<NexoA2AAgentDescriptor> GetAgents();
}

/// <summary>Plain projection of a Nexo agent for A2A purposes.</summary>
...
public sealed record NexoA2AAgentDescriptor(
    string Id,
    string Name,
    string Description,
    string Version,
