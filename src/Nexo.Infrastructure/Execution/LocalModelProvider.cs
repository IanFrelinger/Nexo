...
/// Config: NEXO_LOCAL_MODEL_PATH (required), NEXO_LOCAL_PROVIDER=llama (default).
/// </summary>
public static class LocalModelProvider
{
    ...
    /// <summary>
    /// Returns true when NEXO_LOCAL_MODEL_PATH is set and the model file exists.
    /// </summary>
    ...
    {
        var path = Environment.GetEnvironmentVariable("NEXO_LOCAL_MODEL_PATH")?.Trim();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path))
            throw new ModelUnavailableException(
                "Local model not configured. Set NEXO_LOCAL_MODEL_PATH to a GGUF model file (e.g. llama-3.2-3b-Q4_K_M.gguf).");

        EnsureModelLoaded(path);

        if (_weights == null || _context == null)
            throw new ModelUnavailableException("Failed to load local model. Check NEXO_LOCAL_MODEL_PATH.");
    ...
    private static void EnsureModelLoaded(string path)
    {
        ...
        lock (Lock)
        {
            ...
            try
            {
                var contextSize = int.TryParse(Environment.GetEnvironmentVariable("NEXO_LOCAL_CONTEXT_SIZE"), out var cs) && cs > 0
                    ? cs
                    : 2048;
