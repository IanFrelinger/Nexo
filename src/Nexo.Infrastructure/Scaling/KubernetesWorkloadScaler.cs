...
/// First-class provider — swap to Compose/ECS by changing <c>Nexo:WorkloadScaling:Provider</c>.
/// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        if (!_options.Enabled)
        {
            return new WorkloadScaleResult(
                ...
                "Workload scaling is disabled (Nexo:WorkloadScaling:Enabled=false).");
        }
    ...
    private KubernetesWorkloadBinding RequireBinding(string workloadId)
    {
            ...
            string.IsNullOrWhiteSpace(binding.Deployment))
        {
            throw new KeyNotFoundException(
                $"Unknown Kubernetes workload '{workloadId}'. Configure Nexo:WorkloadScaling:Kubernetes:Workloads.");
        }
