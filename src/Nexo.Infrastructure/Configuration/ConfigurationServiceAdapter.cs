...
/// time using the precedence chain: <c>NEXO_CONFIG_PATH</c> env var → default path
/// <c>~/.nexo/config.json</c> (directory and filename constants live in
/// <see cref="NexoDefaults"/>). This means each adapter instance is bound to a
...
public class ConfigurationServiceAdapter : IConfigurationService
{
    ...
    /// defaults. Corresponds to <c>NexoHostingOptions.StrictMode</c> in the hosting layer.
    /// </param>
    ...
    /// (fresh <c>~/.nexo/</c>) work without manual setup. Failures are wrapped
    ...
    /// <summary>
    /// Returns a <see cref="NexoConfiguration"/> populated with safe production
    /// defaults. Threshold values (e.g. <c>MaxComplexityThreshold</c>,
    /// <c>TimeoutSeconds</c>) are sourced from <see cref="NexoDefaults"/> so they
