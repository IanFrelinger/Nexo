...
/// over the pristine candidate compilation and converts <c>ASHLAR*</c> diagnostics at or above
...
public sealed class AnalyzerFenceGate
{
    ...
    public const string SeverityFloorEnvVar = "ASHLAR_ANALYZER_SEVERITY_FLOOR";
    ...
    /// <c>Ashlar.Analyzers</c>. Explicit construction (not reflection over the assembly) keeps
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (touchSet is not null)
        {
            analyzers = analyzers.Add(new TouchSetReferenceAnalyzer(
                ...
                Ashlar.Core.Application.Autonomy.TrustKernel.KernelNamespacePrefixes));
        }
...
/// <param name="Id">Diagnostic id (<c>ASHLAR*</c>).</param>
...
public sealed record AnalyzerGateOutcome
{
    ...
    /// <summary>Whether manifest-derived rules (ASHLAR0010–0012) were attached for this run (A2).</summary>
    public bool ManifestRulesAttached { get; init; }

    /// <summary>Whether touch-set rules (ASHLAR0013–0014) were attached for this run (autonomy R3.2).</summary>
