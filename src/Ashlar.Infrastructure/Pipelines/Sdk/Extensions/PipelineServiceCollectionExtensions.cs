...
namespace Ashlar.Infrastructure.Pipelines.Sdk.Extensions;
/// <summary>
...
public static class PipelineServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddPipelineCompositionLayer(this IServiceCollection services)
    {
        ...
        {
            if (int.TryParse(Environment.GetEnvironmentVariable("ASHLAR_PIPELINE_MAX_RETRIES"), out var retries) && retries >= 1)
                opts.MaxRetryAttempts = retries;
            if (int.TryParse(Environment.GetEnvironmentVariable("ASHLAR_PIPELINE_RETRY_DELAY_MS"), out var retryDelayMs) && retryDelayMs >= 0)
                opts.RetryDelayMs = retryDelayMs;
            if (TryParseBoolean(Environment.GetEnvironmentVariable("ASHLAR_PIPELINE_RESUME_FAILED"), out var resumeFailed))
                opts.ResumeFailedStages = resumeFailed;
            if (TryParseBoolean(Environment.GetEnvironmentVariable("ASHLAR_PIPELINE_ALLOW_MISSING_RESUME_SOURCE"), out var allowMissingResume))
                opts.AllowMissingResumeSource = allowMissingResume;
            if (TryParseBoolean(Environment.GetEnvironmentVariable("ASHLAR_PIPELINE_ENABLE_TEST_HOOKS"), out var enableTestHooks))
                opts.EnableTestHooks = enableTestHooks;
            var completionPolicy = Environment.GetEnvironmentVariable("ASHLAR_PIPELINE_COMPLETION_POLICY");
        ...
        {
            var provider = Environment.GetEnvironmentVariable("ASHLAR_PIPELINE_STORE_PROVIDER");
            if (!string.IsNullOrWhiteSpace(provider))
                opts.Provider = provider;
            var dbPath = Environment.GetEnvironmentVariable("ASHLAR_PIPELINE_STORE_PATH");
        ...
        {
            var deterministic = Environment.GetEnvironmentVariable("ASHLAR_PIPELINE_DETERMINISTIC_ADAPTER");
            if (!string.IsNullOrWhiteSpace(deterministic))
                opts.DeterministicAdapter = deterministic;
            var agentic = Environment.GetEnvironmentVariable("ASHLAR_PIPELINE_AGENTIC_ADAPTER");
