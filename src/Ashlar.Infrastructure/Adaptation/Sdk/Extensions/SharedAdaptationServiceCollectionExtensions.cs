...
public static class SharedAdaptationServiceCollectionExtensions
{
    ...
    /// <param name="sharedPath">Base path for shared adaptations. Default: ~/.ashlar/shared-adaptations.</param>
        ...
        SharedAdaptationOptions? options = null)
    {
        var peerId = options?.SourcePeerId
            ?? Environment.GetEnvironmentVariable("ASHLAR_MESH_PEER_ID")
        ...
        services.AddSingleton<Ashlar.Core.Application.Adaptation.Ports.ISneakernetTransport>(sp =>
        {
            var sync = sp.GetRequiredService<ISharedAdaptationSync>();
            var logger = sp.GetService<Microsoft.Extensions.Logging.ILogger<Ashlar.Infrastructure.Adaptation.SneakernetTransport>>();
