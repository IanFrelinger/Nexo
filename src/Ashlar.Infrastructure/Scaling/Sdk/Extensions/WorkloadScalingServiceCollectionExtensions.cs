...
namespace Ashlar.Infrastructure.Scaling.Sdk.Extensions;
...
public static class WorkloadScalingServiceCollectionExtensions
{
    ...
    /// Provider selected by <c>Ashlar:WorkloadScaling:Provider</c> or <c>ASHLAR_WORKLOAD_SCALER</c>
    /// (<c>null</c> | <c>kubernetes</c> | <c>compose</c>).
    /// </summary>
    public static IServiceCollection AddAshlarWorkloadScaling(
        ...
        IConfiguration configuration)
    {
        ...
        {
            var envProvider = Environment.GetEnvironmentVariable("ASHLAR_WORKLOAD_SCALER")?.Trim();
            if (!string.IsNullOrWhiteSpace(envProvider))
                opts.Provider = envProvider;

            var envEnabled = Environment.GetEnvironmentVariable("ASHLAR_WORKLOAD_SCALING_ENABLED")?.Trim();
            ...
            var envAutoscale = Environment.GetEnvironmentVariable("ASHLAR_WORKLOAD_AUTOSCALE")?.Trim();
        ...
        var autoscaleEnabled =
            ...
            string.Equals(Environment.GetEnvironmentVariable("ASHLAR_WORKLOAD_AUTOSCALE"), "1", StringComparison.OrdinalIgnoreCase) ||
            string.Equals(Environment.GetEnvironmentVariable("ASHLAR_WORKLOAD_AUTOSCALE"), "true", StringComparison.OrdinalIgnoreCase);
