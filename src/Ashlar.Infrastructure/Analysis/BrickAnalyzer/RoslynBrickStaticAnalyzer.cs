...
public sealed class RoslynBrickStaticAnalyzer : IBrickStaticAnalyzer
{
    ...
    private static List<string> GatherSourceFiles(string path, bool includeAnalyzers)
    {
        ...
        {
            var repoRoot = dir;
            while (repoRoot != null && !File.Exists(Path.Combine(repoRoot, "Ashlar.sln")))
                repoRoot = Path.GetDirectoryName(repoRoot);
            if (repoRoot != null)
            {
                var analysisPath = Path.Combine(repoRoot, "src", "Ashlar.Infrastructure", "Analysis");
                var adaptationPath = Path.Combine(repoRoot, "src", "Ashlar.Infrastructure", "Adaptation");
