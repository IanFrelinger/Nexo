...
public abstract class TempDirTestBase : IDisposable
{
    ...
    /// </summary>
    /// <param name="prefix">Prefix for the temp dir name (e.g. "nexo-dogfood-block1").</param>
    protected TempDirTestBase(string prefix = "nexo-test")
    {
    ...
    {
        if (_disposed) return;
        if (Environment.GetEnvironmentVariable("NEXO_TEST_MEMORY_DIAG") == "1")
        {
            GC.Collect();
