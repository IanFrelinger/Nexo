...
public sealed class CachedValidationServiceAdapterGapCoverageTests
{
    ...
    public async Task ValidateAsync_returns_cached_result_without_calling_inner()
    {
        ...
        inner.Verify(
            s => s.ValidateAsync(It.IsAny<string?>(), It.IsAny<IProgress<Nexo.Core.Application.Common.Models.ProgressReport>?>(), It.IsAny<CancellationToken>()),
            Times.Never);
    }
    ...
    public async Task ValidateAsync_reports_progress_on_cache_hit()
    {
        ...
        var reports = new List<Nexo.Core.Application.Common.Models.ProgressReport>();
        await sut.ValidateAsync(null, new Progress<Nexo.Core.Application.Common.Models.ProgressReport>(reports.Add), CancellationToken.None);
    ...
    public async Task ValidateAsync_stores_result_on_cache_miss()
    {
        ...
        var inner = new Mock<IValidationService>();
        inner.Setup(s => s.ValidateAsync(It.IsAny<string?>(), It.IsAny<IProgress<Nexo.Core.Application.Common.Models.ProgressReport>?>(), It.IsAny<CancellationToken>()))
