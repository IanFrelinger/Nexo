...
namespace Nexo.Tests.Infrastructure.Tests.Onboarding;
...
public sealed class OnboardingE2ETests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "nexo-onboarding-test-" + Guid.NewGuid().ToString("N")[..8]);
    ...
    public async Task ProviderDetection_LocalRequiresModelPath()
    {
        ...
        {
            Environment.SetEnvironmentVariable("NEXO_LOCAL_MODEL_PATH", prevPath);
        }
    }
    ...
    public async Task ConfigResolution_MissingFile_Permissive_ReturnsDefaults()
    {
        ...
        try
        {
            Environment.SetEnvironmentVariable("NEXO_CONFIG_PATH", Path.Combine(_tempDir, "does-not-exist.json"));
            var logger = new LoggerFactory().CreateLogger<ConfigurationServiceAdapter>();
            ...
            config.Validation.TimeoutSeconds.Should().Be(NexoDefaults.ValidationTimeoutSeconds);
        }
        finally
        {
            Environment.SetEnvironmentVariable("NEXO_CONFIG_PATH", prev);
    ...
    public async Task ConfigResolution_MissingFile_StrictMode_Throws()
    {
        ...
        try
        {
            Environment.SetEnvironmentVariable("NEXO_CONFIG_PATH", Path.Combine(_tempDir, "strict-missing.json"));
            var logger = new LoggerFactory().CreateLogger<ConfigurationServiceAdapter>();
    ...
    public async Task ConfigResolution_SaveCreatesDirectory()
    {
        ...
        try
        {
            Environment.SetEnvironmentVariable("NEXO_CONFIG_PATH", nestedPath);
            var logger = new LoggerFactory().CreateLogger<ConfigurationServiceAdapter>();
            var adapter = new ConfigurationServiceAdapter(logger);
            ...
            var loaded = await adapter.LoadAsync();
            loaded.Analysis.MaxComplexityThreshold.Should().Be(NexoDefaults.AnalysisMaxComplexityThreshold);
        }
        finally
        {
            Environment.SetEnvironmentVariable("NEXO_CONFIG_PATH", prev);
    ...
    public async Task HostBootstrap_StrictModeEnabled_PropagatesConfigFailure()
    {
        ...
        try
        {
            Environment.SetEnvironmentVariable("NEXO_CONFIG_PATH", Path.Combine(_tempDir, "host-strict-missing.json"));
            var services = new ServiceCollection();
            services.AddLogging();
    ...
    public async Task ErrorMessages_MockDisabled_IncludesAllowMockHint()
    {
        ...
        try
        {
            ...
            (await act.Should().ThrowAsync<ModelUnavailableException>())
                .Which.Message.Should().Contain("NEXO_ALLOW_MOCK");
        }
        finally
        {
            Environment.SetEnvironmentVariable("NEXO_ALLOW_MOCK", prevMock);
    ...
    public async Task ErrorMessages_StrictConfig_IncludesFilePath()
    {
        ...
        try
        {
            Environment.SetEnvironmentVariable("NEXO_CONFIG_PATH", missingPath);
            var logger = new LoggerFactory().CreateLogger<ConfigurationServiceAdapter>();
            var adapter = new ConfigurationServiceAdapter(logger, failOnConfigWarnings: true);
