...
public class InfrastructureTrustGapCoverageTests
{
    ...
    public async Task CloudAvailabilityResolver_reads_env_var_and_probes_network()
    {
        ...
        var handler = new FakeTrustHandler((_, _) => throw new HttpRequestException("blocked"));
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "nexo-airgap-bad-" + Guid.NewGuid() + ".json");
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "nexo-airgap-" + Guid.NewGuid() + ".json");
