...
using Nexo.Infrastructure.Certification.Probes;
...
public sealed class DiagnosticProbeTests
{
    ...
    public async Task AnalyzerFailure_GroupsFindingsByRule_AndNonCompilingNamesTheFirstError()
    {
        ...
        grouped.Facts.Should().ContainKey("NEXO0006");
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"nexo-probe-{Guid.NewGuid():N}.csproj");
