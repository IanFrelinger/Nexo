...
public sealed class AnalyzerGateAdversarialCampaignTests
{
    ...
    public async Task Attack_alias_dodge_of_a_forbidden_api_never_certifies()
    {
        ...
        decision.Record.Reason.Should().Contain("NEXO0011",
    ...
    public async Task Attack_first_line_violation_cannot_hide_behind_the_wrapper_exemption()
    {
        ...
        var manifest = new BrickConstraintManifest { AllowedUsings = ["Nexo.Core.Domain.Bricks"] };
        var attack = """
            using System.Text;
        ...
        var finding = outcome.Findings.First(
            f => f.Id == Nexo.Analyzers.BrickConstraintManifestAnalyzer.DisallowedUsingId);
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"nexo-cert-adversarial-{Guid.NewGuid():N}.csproj");
