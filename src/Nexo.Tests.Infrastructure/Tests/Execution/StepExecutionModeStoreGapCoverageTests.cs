...
public sealed class StepExecutionModeStoreGapCoverageTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "nexo-modes-" + Guid.NewGuid().ToString("N") + ".json");
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "nexo-modes-bad-" + Guid.NewGuid().ToString("N") + ".json");
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "nexo-modes-save-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "nexo-modes-swap-" + Guid.NewGuid().ToString("N") + ".json");
        try
        {
            var store = new StepExecutionModeStore(path, NullLogger<StepExecutionModeStore>.Instance);
