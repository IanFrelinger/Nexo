...
public sealed class PathAllowlistGapCoverageTests
{
    ...
    {
        Environment.SetEnvironmentVariable("NEXO_PATH_ALLOWLIST_EXTRA", "generated/, staging");
        try
        {
        ...
        {
            Environment.SetEnvironmentVariable("NEXO_PATH_ALLOWLIST_EXTRA", null);
        }
    }

    [Fact]
    public void Approve_allows_nexo_metadata_directory()
    {
        var policy = new PathAllowlist();
        var call = CreateToolCall("repo.fs.write", ".nexo/state.json");
    ...
    {
        Environment.SetEnvironmentVariable("NEXO_SANDBOX_ROOT", "\0invalid");
        try
        {
            var policy = new PathAllowlist();
    ...
    {
        var sandboxRoot = Path.Combine(Path.GetTempPath(), "nexo-sandbox-root-" + Guid.NewGuid().ToString("N"));
