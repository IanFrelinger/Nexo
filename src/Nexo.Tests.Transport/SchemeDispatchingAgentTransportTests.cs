...
public sealed class SchemeDispatchingAgentTransportTests
{
    ...
    [Fact]
    public async Task AddNexoRuntimeTransport_wraps_remote_when_scheme_registrations_exist()
    {
        var services = new ServiceCollection();
        services.AddLogging();
        var a2a = new RecordingTransport("a2a");
        services.AddSingleton(new AgentTransportSchemeRegistration("a2a+", _ => a2a));
        services.AddNexoRuntimeTransport<RecordingInProcess, RecordingRemote>();
    ...
    [Fact]
    public async Task AddNexoRuntimeTransport_without_registrations_keeps_previous_composition()
    {
        var services = new ServiceCollection();
        services.AddLogging();
        services.AddNexoRuntimeTransport<RecordingInProcess, RecordingRemote>();
