...
public static class ForgeMapFetchUrlValidator
{
    public static bool TryValidate(string rawUrl, ForgeSessionOptions options, out string? error)
    {
        ...
        {
            error = "http URLs are disabled; use https or set Nexo:ForgeSession:AllowInsecureMapFetch.";
            return false;
        }
        ...
        {
            error = "No allowed map fetch hosts configured (Nexo:ForgeSession:AllowedMapFetchHosts).";
            return false;
        }

        if (allowed.Count > 0 && !HostMatchesAllowlist(host, allowed))
        {
            error = $"Host '{host}' is not in Nexo:ForgeSession:AllowedMapFetchHosts.";
            return false;
        }
