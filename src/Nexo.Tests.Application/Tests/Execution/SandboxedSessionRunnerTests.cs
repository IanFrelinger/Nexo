...
/// <summary>
/// Session sandboxes (extension spec Part B): a session is started detached with name +
/// deadline labels, work goes through <c>docker exec</c>, teardown is guaranteed on every
/// disposal path and idempotent, use-after-teardown is loud, and the reaper sweeps
/// leaked containers by deadline — fail-closed for sessions whose deadline is unreadable.
/// </summary>
