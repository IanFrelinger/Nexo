...
public class CoreApplicationGapTests
{
    ...
    [Fact]
    public void NoOpAshlarIngressAccessor_returns_nulls()
    {
        var accessor = new NoOpAshlarIngressAccessor();
    ...
    {
        new Ashlar.Core.Application.Knowledge.Models.KnowledgeQueryRequest
        {
            Since = DateTimeOffset.UtcNow.AddDays(-1),
            DataType = "pattern",
            MaxCount = 5,
            Offset = 1,
            Sources = new[] { Ashlar.Core.Application.Knowledge.Models.KnowledgeSource.Pattern },
        }.MaxCount.Should().Be(5);

        new Ashlar.Core.Application.Knowledge.Models.KnowledgeQueryEntry
        {
            Id = "k1",
            Source = Ashlar.Core.Application.Knowledge.Models.KnowledgeSource.Adaptation,
            Timestamp = DateTimeOffset.UtcNow,
            Summary = "snippet",
        }.Summary.Should().Be("snippet");

        new Ashlar.Core.Application.Execution.Models.HotSwapResult(
                "step-1",
                Ashlar.Core.Application.Execution.Models.ExecutionMode.Deterministic,
                Ashlar.Core.Application.Execution.Models.ExecutionMode.Agentic,
                true,
                null,
                DateTimeOffset.UtcNow)
            .Success.Should().BeTrue();

        new Ashlar.Core.Application.Adaptation.ImmutableCoreViolationException("/core/file.cs")
    ...
    public void Workflow_execution_metrics_and_events_round_trip()
    {
        ...
        new NodeMetrics
        {
            ...
            Implementation = Ashlar.Core.Domain.Bricks.ImplementationType.Deterministic,
            TokensUsed = 10,
            CacheHit = true,
        }.CacheHit.Should().BeTrue();

        new BrickImplementationSelectedEvent("c1", "n1", "Echo", Ashlar.Core.Domain.Bricks.ImplementationType.Agentic)
            .BrickName.Should().Be("Echo");
        new BrickImplementationSelectedEvent("c1", "n1", "Echo", Ashlar.Core.Domain.Bricks.ImplementationType.Agentic)
            .NodeId.Should().Be("n1");
        new BrickImplementationSelectedEvent("c1", "n1", "Echo", Ashlar.Core.Domain.Bricks.ImplementationType.Agentic)
            .Implementation.Should().Be(Ashlar.Core.Domain.Bricks.ImplementationType.Agentic);
        ...
        new NodeBrickEvent("c1", "n1", new StepCompletedEvent("s1", "b1", Ashlar.Core.Domain.Bricks.ImplementationType.Deterministic, 1))
            .NodeId.Should().Be("n1");
    }
    ...
    {
        var ingress = new Mock<IAshlarIngressAccessor>();
        ...
        var behavior = new Ashlar.Core.Application.Behaviors.IngressLoggingPipelineBehavior<DummyRequest, string>(
            ingress.Object,
            NullLogger<Ashlar.Core.Application.Behaviors.IngressLoggingPipelineBehavior<DummyRequest, string>>.Instance);
    ...
    {
        new Ashlar.Core.Application.Testing.Abstractions.AssertionException("assert failed")
    ...
    public async Task RunTestsHandler_runs_cleanup_before_and_after_when_configured()
    {
        ...
        runner.Setup(r => r.RunTestsAsync(It.IsAny<string?>(), It.IsAny<IProgress<Ashlar.Core.Application.Common.Models.ProgressReport>>(), It.IsAny<CancellationToken>()))
    ...
    {
        var runner = new Mock<ITestRunner>();
        runner.Setup(r => r.RunTestsAsync(null, It.IsAny<IProgress<Ashlar.Core.Application.Common.Models.ProgressReport>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new TestExecutionResult
            {
                TotalTests = 0,
                PassedTests = 0,
