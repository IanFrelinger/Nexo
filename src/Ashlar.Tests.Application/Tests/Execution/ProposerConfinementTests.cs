...
public sealed class ProposerConfinementTests
{
    ...
    {
        var confinement = new ProposerConfinement { WritablePrefixes = new[] { "src/", ".ashlar" } };

        var allowlist = confinement.ToPathAllowlistPrefixes();
        var mounts = confinement.ToMounts("C:\\repo");

        allowlist.Should().Equal("src/", ".ashlar/");
        ...
        mounts.Skip(1).Select(m => m.ContainerPath).Should().Equal("/workspace/src", "/workspace/.ashlar");
        mounts.Skip(1).Select(m => m.HostPath).Should().Equal("C:/repo/src", "C:/repo/.ashlar");
    }
    ...
    // integration are pinned in Ashlar.Tests.Infrastructure (DockerSandboxSessionTests),
