...
namespace Ashlar.Tests.Application.Ide;
...
public sealed class IdeEndpointSurfaceConventionTests
{
    private static string FindRepoRoot()
    {
        ...
        while (dir is not null)
        {
            if (File.Exists(Path.Combine(dir.FullName, "Ashlar.sln"))
                || File.Exists(Path.Combine(dir.FullName, "application", "src", "Ashlar.API", "Endpoints", "IdeEndpoints.cs")))
                return dir.FullName;
            dir = dir.Parent;
        }
    ...
    {
        var path = Path.Combine(FindRepoRoot(), "application", "src", "Ashlar.API", "Endpoints", "IdeEndpoints.cs");
