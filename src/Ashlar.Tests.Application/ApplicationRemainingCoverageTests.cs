...
public sealed class ApplicationRemainingCoverageTests
{
    ...
    public void MeshTrustPolicyConfiguration_normalizes_and_resolves_env()
    {
        ...
        try
        {
            Environment.SetEnvironmentVariable("ASHLAR_MESH_TRUST_POLICY", "any");
            Environment.SetEnvironmentVariable("ASHLAR_PEER_TRUST_POLICY", null);
            MeshTrustPolicyConfiguration.ResolveDiscoveryPolicy().Should().Be("any");

            Environment.SetEnvironmentVariable("ASHLAR_MESH_TRUST_POLICY", null);
            Environment.SetEnvironmentVariable("ASHLAR_PEER_TRUST_POLICY", "trusted-only");
        ...
        {
            Environment.SetEnvironmentVariable("ASHLAR_MESH_TRUST_POLICY", mesh);
            Environment.SetEnvironmentVariable("ASHLAR_PEER_TRUST_POLICY", peer);
        }
    }
    ...
    public void InstanceCapabilities_dedupes_formats_and_exposes_static_profiles()
    {
        var caps = new InstanceCapabilities(
            ...
            isAirGapped: true,
            availableComponents: new[] { "ashlar-cli" });
        ...
        InstanceCapabilities.LocalAshlar.CanCompile.Should().BeTrue();
        InstanceCapabilities.LocalAshlar.AvailableComponents.Should().Contain("ashlar-cli");
    }
