...
public sealed class SelfExtendInvariantACertGateTests
{
    ...
    private const string BrickRelativePath = "src/Ashlar.Bricks.AuditGap/UncertifiedSelfProposedBrick.cs";
    private const string BrickSource =
        "namespace Ashlar.Bricks.AuditGap; public sealed class UncertifiedSelfProposedBrick { }";
    ...
    public void Characterization_selfExtend_policy_chain_includes_certification_gate_when_store_wired()
    {
        var (_, policies, _) = RepoFsToolboxFactory.CreateWithBuildTest(
            certificationStore: new Ashlar.Infrastructure.Certification.InMemoryCertificationRecordStore());
