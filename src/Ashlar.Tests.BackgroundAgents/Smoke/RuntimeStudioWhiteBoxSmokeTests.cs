...
namespace Ashlar.Tests.BackgroundAgents.Smoke;

/// <summary>
/// White-box smoke: exercises runtime-studio stores and forge policy in-process
/// (no CLI, no daemon). Complements <see cref="Ashlar.Tests.Infrastructure.Tests.CLI.RuntimeStudioBlackBoxSmokeTests"/>.
/// </summary>
...
public sealed class RuntimeStudioWhiteBoxSmokeTests : IDisposable
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "ashlar-whitebox-smoke-" + Guid.NewGuid().ToString("N"));
