...
public class ObservationPipelineServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "ashlar-obs-pipe-" + Guid.NewGuid().ToString("N"));
    ...
    private sealed class DenyAllObservationGate : Ashlar.Core.Application.Trust.Ports.IObservationGate
    {
    ...
    private sealed class ThrowingPatternStore : Ashlar.Core.Application.Observation.Ports.IPatternStore
    {
        public Task AddAsync(Ashlar.Core.Application.Observation.Models.ObservedPattern pattern, CancellationToken cancellationToken = default)
            => throw new UnauthorizedAccessException("test fail-open");

        public Task<IReadOnlyList<Ashlar.Core.Application.Observation.Models.ObservedPattern>> QueryAsync(
            Ashlar.Core.Application.Observation.Models.PatternStoreQueryParams query,
