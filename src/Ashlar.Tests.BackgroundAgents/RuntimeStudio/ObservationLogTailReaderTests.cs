...
public sealed class ObservationLogTailReaderTests : IDisposable
{
    ...
    {
        _path = Path.Combine(Path.GetTempPath(), "ashlar-obs-tail-" + Guid.NewGuid().ToString("N") + ".jsonl");
    }
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "ashlar-obs-dir-" + Guid.NewGuid().ToString("N"));
