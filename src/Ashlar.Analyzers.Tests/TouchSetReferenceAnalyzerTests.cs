...
{
    private static Ashlar.Analyzers.TouchSetReferenceAnalyzer Analyzer(
    ...
    {
        // System*, threading, tasks, and the Ashlar.Core.Domain contracts are the floor
    ...
    public async Task ReferenceOutsideTheDeclaration_IsFlagged()
    {
        ...
        var finding = diagnostics.Should().ContainSingle(
            d => d.Id == Ashlar.Analyzers.TouchSetReferenceAnalyzer.OutsideTouchSetId).Subject;
    ...
    public async Task UndeclaredKernelReference_GetsTheSmugglingDiagnostic()
    {
        ...
        var finding = diagnostics.Should().ContainSingle(
            d => d.Id == Ashlar.Analyzers.TouchSetReferenceAnalyzer.UndeclaredKernelReferenceId).Subject;
    ...
    public async Task BaseTypesCount_AsReferences()
    {
        ...
        diagnostics.Should().Contain(
            d => d.Id == Ashlar.Analyzers.TouchSetReferenceAnalyzer.OutsideTouchSetId);
    }
    ...
    public async Task ReflectionStrings_AreHonestlyOutOfScope()
    {
        ...
        var source = BrickSource(body: """
            var name = "Ashlar.Policies.PolicyEngine";
            output.Set("t", name);
            """);

        var diagnostics = await AnalyzeAsync(
            source, Analyzer(kernel: new[] { "Ashlar.Policies" }));
