...
/// <summary>
/// Projects Ashlar agent descriptors onto A2A-spec agent cards.
///
/// Naming note: <c>A2A.AgentCard</c> (the spec discovery document) collides with Ashlar's own
/// <c>Ashlar.Core.Domain.Agents.AgentCard</c> persona model. The spec type is aliased and never
/// leaves the A2A adapter projects; the domain type is untouched.
/// </summary>
public interface IAshlarA2ACardProjector
{
    /// <summary>Builds the spec card for one exposed agent.</summary>
    A2ACard Project(AshlarA2AAgentDescriptor descriptor, AshlarA2AServerOptions options, string basePattern);
}

/// <summary>Default projector.</summary>
public sealed class AshlarA2ACardProjector : IAshlarA2ACardProjector
{
    private static readonly List<string> DefaultModes = ["text/plain", "application/json"];

    /// <inheritdoc />
    public A2ACard Project(AshlarA2AAgentDescriptor descriptor, AshlarA2AServerOptions options, string basePattern)
    {
        ...
        return new A2ACard
        {
            ...
            // AshlarA2AServerOptions class docs.
