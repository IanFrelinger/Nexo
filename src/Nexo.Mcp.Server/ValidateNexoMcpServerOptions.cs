...
/// <summary>
/// Fail-closed startup validation for <see cref="NexoMcpServerOptions"/> (wired via
/// <c>ValidateOnStart</c> so a misconfigured host refuses to boot instead of serving a
/// half-configured protocol surface).
/// </summary>
public sealed class ValidateNexoMcpServerOptions : IValidateOptions<NexoMcpServerOptions>
{
    ...
    /// <inheritdoc />
    public ValidateOptionsResult Validate(string? name, NexoMcpServerOptions options)
    {
        if (!options.Enabled)
        {
        ...
        {
            failures.Add(
                $"{nameof(NexoMcpServerOptions.Enabled)}=true is not permitted under the AirGapped deployment profile " +
        ...
        {
            failures.Add($"{nameof(NexoMcpServerOptions.ServerName)} must be non-empty when the MCP server is enabled.");
        }

        if (options.MaxConcurrentToolCalls < 1)
        {
            failures.Add($"{nameof(NexoMcpServerOptions.MaxConcurrentToolCalls)} must be >= 1 (got {options.MaxConcurrentToolCalls}).");
        }
