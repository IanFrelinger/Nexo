...
public sealed class BackgroundAgentRegistryGapCoverageTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "nexo-cycles-" + Guid.NewGuid().ToString("N") + ".jsonl");
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "nexo-cycles-fail-" + Guid.NewGuid().ToString("N") + ".jsonl");
        var cycleEvents = new CycleEventStore(path);
