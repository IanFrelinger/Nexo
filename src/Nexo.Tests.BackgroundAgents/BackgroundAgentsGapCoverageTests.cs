...
public class BackgroundAgentsGapCoverageTests
{
    ...
    public async Task BingWebSearchProvider_parses_successful_bing_response()
    {
        const string json = """
            {
              "webPages": {
                "value": [
                  { "name": "Nexo docs", "url": "https://example.com/nexo", "snippet": "kernel transport" },
        ...
        var results = await provider.SearchAsync("nexo", 1);

        results.Should().ContainSingle();
        results[0].Title.Should().Be("Nexo docs");
        results[0].Url.Should().Be("https://example.com/nexo");
