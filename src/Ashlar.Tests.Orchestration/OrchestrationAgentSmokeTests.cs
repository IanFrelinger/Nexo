...
public class OrchestrationAgentSmokeTests
{
    ...
    {
        var tempFile = Path.Combine(Path.GetTempPath(), "ashlar-img-" + Guid.NewGuid() + ".png");
    ...
    {
        var tempFile = Path.Combine(Path.GetTempPath(), "ashlar-aud-" + Guid.NewGuid() + ".wav");
    ...
    {
        var tempFile = Path.Combine(Path.GetTempPath(), "ashlar-3d-" + Guid.NewGuid() + ".glb");
        await File.WriteAllBytesAsync(tempFile, new byte[] { 1, 2, 3 });
