...
/// - honors "nexo.model.prefer=deterministic" directive in system messages
    ...
    private static string? ParsePrefer(ModelInput input)
    {
        foreach (var (role, content) in input.Messages)
        {
            ...
            foreach (var line in (content ?? "").Split('\n'))
            {
                ...
                {
                    return trimmed["nexo.model.prefer=".Length..].Trim();
                }
            }
        }
        return null;
    ...
    private static string? ParseProvider(ModelInput input)
    {
        foreach (var (role, content) in input.Messages)
        {
            ...
            foreach (var line in (content ?? "").Split('\n'))
            {
                ...
                {
                    return trimmed["nexo.model.provider=".Length..].Trim();
                }
            }
        }
        return null;
