...
using Xunit;
// MEAI 10.9.0 ships its own Microsoft.Extensions.AI.RoutingChatClient; alias the Ashlar router
...
public sealed class RoutingChatClientTests
{
    ...
    public async Task Policy_forbids_cloud_never_consults_cloud_spy()
    {
        ...
        services.AddAshlarGovernedChatClient(MeaiTargetKeys.LocalOllama, _ => localSpy);
        services.AddAshlarGovernedChatClient(MeaiTargetKeys.LocalOnnx, _ => new SpyChatClient("onnx"));
        services.AddAshlarGovernedChatClient(DefaultRouteCandidateTable.CloudBedrockBalanced, _ => cloudSpy);
    ...
    public async Task Local_down_cloud_forbidden_fails_without_silent_degradation()
    {
        ...
        services.AddAshlarGovernedChatClient(MeaiTargetKeys.LocalOllama, _ => new SpyChatClient("local"));
        services.AddAshlarGovernedChatClient(MeaiTargetKeys.LocalOnnx, _ => new SpyChatClient("onnx"));
        services.AddAshlarGovernedChatClient(DefaultRouteCandidateTable.CloudBedrockBalanced, _ => cloudSpy);
        services.AddSingleton(sp => ActivatorUtilities.CreateInstance<RoutingChatClient>(sp));
