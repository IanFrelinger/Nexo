...
4. **M2 — Tile orchestration:** Uses **`WebMercatorTileMath`** + **`VectorTileUrlBuilder`** (from `Ashlar.Commercial.GameDomain`) to build a Mapbox vector tile URL from lon/lat/zoom + **`MAPBOX_ACCESS_TOKEN`**.
5. **Phase B — Tile disk cache:** When **`ASHLAR_TILE_CACHE_DIR`** is set, writes raw MVT bytes under **`MapTileDiskCache`** keyed by **`MapTileCacheKey`** (aesthetic + provider + **`z/x/y`**).
...
- **Ashlar.Commercial.GameDirector.Host** running locally (or set **`ASHLAR_API_BASE_URL`** to your Forge deployment).
- Optional: **Mapbox access token** for live tile fetch — must match **`Ashlar:ForgeSession:AllowedMapFetchHosts`** on the Game Director host (include `api.mapbox.com`).
...
export ASHLAR_API_BASE_URL="http://127.0.0.1:8080"
...
|----------|---------|---------|
| `ASHLAR_API_BASE_URL` | `http://127.0.0.1:8080` | Game Director host base URL (Forge `/api/forge/*`) |
...
| `ASHLAR_TILE_CACHE_DIR` | _(empty)_ | When set, enables **`MapTileDiskCache`** demo after building the Mapbox URL (phase B). |
