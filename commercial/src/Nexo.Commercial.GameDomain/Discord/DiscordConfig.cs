...
public sealed record DiscordConfig
{
    ...
    public static DiscordConfig Load(string projectRoot)
    {
        ...
        if (!System.IO.File.Exists(path))
            ...
            {
                WebhookUrl = Environment.GetEnvironmentVariable("NEXO_DISCORD_WEBHOOK_URL"),
                BotToken = Environment.GetEnvironmentVariable("NEXO_DISCORD_BOT_TOKEN"),
                ChannelId = Environment.GetEnvironmentVariable("NEXO_DISCORD_CHANNEL_ID"),
                OwnerUserId = Environment.GetEnvironmentVariable("NEXO_DISCORD_OWNER_ID")
            };
