...
{
    public const string SectionPath = "Ashlar:ForgeSession";
    ...
    /// <summary>
    /// When true, non-dry map pipeline runs may call <see cref="Ashlar.Commercial.GameDomain.Mapping.IVectorMapIntelligenceService"/> on fetched vector bytes.
    /// </summary>
    public bool EnableVectorIntelligence { get; set; }

    /// <summary>
    /// When true and <see cref="EnableVectorIntelligence"/> is true, runs a short <see cref="Ashlar.Abstractions.IModel"/> prompt after heuristics.
    /// </summary>
    ...
    /// <summary>
    /// When true, requests under <c>/api/forge</c> must be authenticated (via Ashlar built-in auth).
    /// </summary>
    ...
    /// <summary>
    /// When true (default), <see cref="MapPipelineRunner"/> runs <see cref="Ashlar.Commercial.GameDomain.Mapping.IMapVerificationService"/> after vector parse.
    /// </summary>
