...
public class FileVectorMapSourceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "nexo-map-test-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "nexo-osm-test-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        try
        {
            var rel = "mini.osm";
