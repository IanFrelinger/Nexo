...
{
    public Compounding_AddTranslate_BenefitsTwoRecipes()
    {
    ...
    public async Task Adding_Translate_Improves_Triage_And_Doc_Summary()
    {
        ...
        var triageEnv = new Dictionary<string, string>
        {
            ["NEXO_AI_MODE"] = "hybrid",
            ["NEXO_PROVIDER"] = "openai"
        };
        
        var triageResult = await DemoHarness.RunScenarioAsync("recipes/triage_support.yaml", triageEnv);
        
        // Act - Run document summarization recipe (should also benefit from Translate block)
        var docSummaryResult = await DemoHarness.RunScenarioAsync("recipes/doc_summarize.yaml", triageEnv);
    ...
    public async Task Translate_Block_Works_With_Multiple_Languages()
    {
        ...
        var spanishEnv = new Dictionary<string, string>
        {
            ["NEXO_AI_MODE"] = "hybrid",
            ["NEXO_PROVIDER"] = "openai",
            ["NEXO_INPUT_LANGUAGE"] = "es"
        };
        var spanishResult = await DemoHarness.RunScenarioAsync("recipes/triage_support.yaml", spanishEnv);

        // Test with French content
        var frenchEnv = new Dictionary<string, string>
        {
            ["NEXO_AI_MODE"] = "hybrid",
            ["NEXO_PROVIDER"] = "openai",
            ["NEXO_INPUT_LANGUAGE"] = "fr"
        };
        var frenchResult = await DemoHarness.RunScenarioAsync("recipes/triage_support.yaml", frenchEnv);
    ...
    public async Task Existing_Recipes_Automatically_Benefit_From_New_Blocks()
    {
        ...
        var triageResult = await DemoHarness.RunScenarioAsync("recipes/triage_support.yaml", env);
        var docResult = await DemoHarness.RunScenarioAsync("recipes/doc_summarize.yaml", env);
        var emailResult = await DemoHarness.RunScenarioAsync("recipes/email_classification.yaml", env);
