...
public partial class Export_CLI_And_Docker_Smoke
{
    ...
    private async Task<bool> ExportCliArtifactAsync(string exportDir, bool includeOfflineMode = false, bool includeSamples = false)
    {
        ...
        // 1. Call the Nexo export CLI API
        // 2. Package the CLI with all dependencies
        // 3. Include offline mode if requested
        // 4. Include sample recipes if requested
        
        // Simulate creating CLI files
        var cliPath = Path.Combine(exportDir, "nexo-cli");
        await File.WriteAllTextAsync(cliPath, "#!/bin/bash\necho 'Nexo CLI exported'");
