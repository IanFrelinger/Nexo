...
{
    /// <summary>
    /// Headless tests for adapter health checks.
    /// These tests can run in CI environments without Unity Editor.
    /// </summary>
    [TestFixture]
    [Category("Headless")]
    public class Headless_AdapterHealth
    {
        ...
        {
            // Arrange
            var unreachableAdapter = new OllamaAdapter();
        ...
        {
            // Arrange
            var unreachableAdapter = new ComfyUIAdapter();
        ...
        {
            // Arrange
            var unreachableAdapter = new PiperAdapter();
        ...
        public async Task OllamaAdapter_ShouldHandleCancellation()
        {
            ...
            {
                _ollamaAdapter.HealthCheckAsync(cancellationTokenSource.Token).Wait();
            });
        }
        ...
        public async Task ComfyUIAdapter_ShouldHandleCancellation()
        {
            ...
            Assert.Throws<OperationCanceledException>(() =>
            {
                _comfyuiAdapter.HealthCheckAsync(cancellationTokenSource.Token).Wait();
            });
        }
        ...
        public async Task PiperAdapter_ShouldHandleCancellation()
        {
            ...
            Assert.Throws<OperationCanceledException>(() =>
            {
                _piperAdapter.HealthCheckAsync(cancellationTokenSource.Token).Wait();
            });
        }
