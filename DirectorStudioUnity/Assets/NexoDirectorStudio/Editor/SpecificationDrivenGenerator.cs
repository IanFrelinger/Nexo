...
{
    /// <summary>
    /// Specification-driven game generator that uses detailed game specs
    /// to create more targeted and coherent game content.
    /// </summary>
    public static class SpecificationDrivenGenerator
    {
        // Usage: -executeMethod NexoDirectorStudio.Editor.SpecificationDrivenGenerator.GenerateFromSpec
        //        [-specPath /path/to/game-specification.json] [-includePlaytest true]
        public static void GenerateFromSpec()
        {
            ...
            GenerateFromSpecification(specPath, includePlaytest, resultsPath);
        }
        
        private static IEnumerator GenerateFromSpecification(string specPath, bool includePlaytest, string resultsPath)
        {
            Debug.Log("🎮 === SPECIFICATION-DRIVEN GAME GENERATION ===");
...
Focus on creating a cohesive experience that delivers the {spec.EmotionalGoal.ToLower()} through {spec.CoreInteraction.ToLower()} in a {spec.Setting.ToLower()} world.";
        ...
        private static IEnumerator RunSpecificationPlaytest(string scenePath, GameSpecification spec)
        {
            // Load the scene
            ...
            var agentGO = new GameObject("SpecificationPlaytestAgent");
            var agent = agentGO.AddComponent<SimplePlaytestAgent>();
            agent.testDuration = 10f; // Longer test for specification validation
            ...
            // Wait for playtest completion
            var startTime = Time.time;
            while (!completed && (Time.time - startTime) < 20f)
            {
            ...
            yield return result ?? new PlaytestMetrics { playtestCompleted = false };
        }
