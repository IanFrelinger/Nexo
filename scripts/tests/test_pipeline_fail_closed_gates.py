...
class TestMultiEnvCommandFailClosedTests(unittest.TestCase):
    def test_test_multi_env_command_fails_closed_on_zero_tests(self) -> None:
        text = (
            ROOT / "application" / "src" / "Ashlar.CLI" / "Commands" / "TestMultiEnvCommand.cs"
        ).read_text(encoding="utf-8")
        self.assertIn("DotnetTestTool.HasExecutedTests", text)
        self.assertIn("EnvRunPassed", text)
        self.assertNotIn("passed == 0 && total == 0 && runExit != 0", text)
        self.assertNotIn("if (runExit != 0) failed++;", text)
...
class DotNetRegressionTestRunnerFailClosedTests(unittest.TestCase):
    def test_regression_runner_fails_closed_on_zero_tests(self) -> None:
        text = (
            ROOT
            / "src"
            ...
            / "Analysis"
            / "BrickAnalyzer"
            / "DotNetRegressionTestRunner.cs"
        ).read_text(encoding="utf-8")
        self.assertIn("failed == 0 && passed >= 1", text)
        self.assertIn("No tests matched the filter", text)


class ValidationServiceAdapterFailClosedTests(unittest.TestCase):
    def test_validation_adapter_fails_closed_on_zero_tests(self) -> None:
        text = (
            ROOT
            / "src"
            / "Ashlar.Infrastructure"
        ...
        self.assertIn("totalTestsFailed == 0 && totalTestsRun >= 1", text)
...
class MakefileSlnMinFloorTests(unittest.TestCase):
    def test_makefile_runs_min_floor_on_full_solution(self) -> None:
        makefile = (ROOT / "Makefile").read_text(encoding="utf-8")
        self.assertIn("run-dotnet-test-min-floor.py", makefile)
        self.assertIn("--min-listed 4472", makefile)
        self.assertIn("--project Ashlar.sln", makefile)
        self.assertNotIn(
