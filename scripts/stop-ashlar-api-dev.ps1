...
<#
.SYNOPSIS
  Stop Docker Ollama from deploy/compose/docker-compose.ollama.yml; optionally free the Ashlar API port.

.PARAMETER KeepOllama
  If set, only attempt to stop the API listener (no docker compose down).

.PARAMETER ApiPort
  Port Ashlar.API was bound to (default: 8080). Used only with -KillApi.
