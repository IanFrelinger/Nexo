...
VERSION="${ASHLAR_SDK_PACKAGE_VERSION:-0.0.0-kernel-gate-local}"
export ASHLAR_SDK_PACKAGE_VERSION="${VERSION}"

echo "== Tier D: runtime graph builds without application hosts =="
dotnet build Ashlar.Runtime.sln -v minimal

echo "== Tier D: pack list vs Ashlar.Hosting MSBuild closure =="
