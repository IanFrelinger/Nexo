...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-ashlar_mesh_lab_tls_local}"
export COMPOSE_PROJECT_NAME

ENV_FILE="${1:-}"
TEMP_ENV=false
if [[ -z "$ENV_FILE" || ! -f "$ENV_FILE" ]]; then
  ENV_FILE="$(mktemp "${TMPDIR:-/tmp}/ashlar-mesh-lab-tls.XXXXXX.env")"
  TEMP_ENV=true
  KEY="$(openssl rand -hex 24 2>/dev/null || printf 'meshlabtls%d%s' "$RANDOM" "$$")"
  {
    echo "Ashlar__Security__ApiKey=${KEY}"
    echo "Ashlar__Security__PeerB__BearerToken=${KEY}-bearer"
    echo "Ashlar__Security__Worker__BasicAuthPassword=${KEY}-basic"
