...
run_restore() {
  ensure_repo_files
  if ! has_command dotnet; then
    echo "dotnet not found. Install .NET SDK 8+ via your IDE, then re-run setup check/restore." >&2
    return 1
  fi
...
print_missing_guidance() {
  ...
  case "${dep}" in
    ...
    dotnet) echo "Install .NET SDK 8+ using your IDE installer (recommended)." ;;
...
ensure_dotnet_ready_or_install() {
  ...
  echo "  [INSTALL] dotnet SDK >= 8 via Homebrew"
