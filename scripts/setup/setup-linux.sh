...
install_dotnet_sdk() {
  ...
  if [[ "${PKG_MANAGER}" == "brew" ]]; then
    confirm_or_exit "Install .NET SDK 8.x using Homebrew?"
    brew install dotnet-sdk
    ensure_dotnet_path
    if ! has_supported_dotnet; then
      echo "Failed to install .NET SDK 8.x via Homebrew." >&2
      return 1
    fi
    return
  fi
  ...
  confirm_or_exit "Install .NET SDK 8.x to ${HOME}/.dotnet?"
  ...
  if ! has_supported_dotnet; then
    echo "Failed to install .NET SDK 8.x automatically." >&2
    return 1
  fi
}
...
run_restore() {
  ensure_repo_files
  if ! has_supported_dotnet; then
    echo "dotnet SDK 8+ not found. Run apply mode first: bash scripts/setup/setup-linux.sh apply --yes" >&2
    return 1
  fi
...
print_missing_guidance() {
  ...
  case "${dep}" in
    ...
    dotnet) echo "Run: bash scripts/setup/setup-linux.sh apply --yes (auto-installs .NET SDK 8 locally)." ;;
...
check_dependencies() {
  ...
  if has_supported_dotnet; then
    echo "  [OK] dotnet SDK >= 8"
  else
    echo "  [MISSING] dotnet SDK >= 8"
...
apply_dependencies() {
  guided_step "Checking and installing required developer tools (git, curl, .NET SDK 8+) if missing."
  if ! has_command git; then
  ...
  if ! has_supported_dotnet; then
    guided_step ".NET SDK 8 is missing; installing it now."
    install_dotnet_sdk
  fi
