...
function Invoke-Restore {
    ...
    if (-not (Test-CommandExists -Name "dotnet")) {
        throw "dotnet not found. Install .NET SDK 8+ via your IDE, then re-run setup check/restore."
    }
...
function Invoke-DependencyCheck {
    ...
    if (Test-SupportedDotnet) {
        Write-Host "  [OK] dotnet SDK >= 8"
    } else {
        Write-Host "  [MISSING] dotnet SDK >= 8"
    ...
    if ($missingRequired.Count -gt 0) {
        foreach ($dep in $missingRequired) {
            switch ($dep) {
                ...
                "dotnet" { Write-Host "  - Install .NET SDK 8+ via your IDE installer (recommended)." }
