# Package Nexo VS Code extension as a .vsix (no npm/vsce required — plain JS zip).
# Output: extensions/nexo-vscode/nexo-<version>.vsix
param(
    [string]$OutDir = ""
)
...
$name = $pkg.name
$version = $pkg.version
$publisher = if ($pkg.publisher) { $pkg.publisher } else { "nexo" }
...
$staging = Join-Path $env:TEMP ("nexo-vsix-" + [guid]::NewGuid().ToString("N"))
