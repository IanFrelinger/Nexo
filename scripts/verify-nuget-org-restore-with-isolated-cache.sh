...
# Same env: NEXO_NUGET_RESTORE_VERIFY_VERSION (required). Optional NEXO_NUGET_VERIFY_NO_ISOLATED_CACHE=1 to use caller env.
set -euo pipefail
...
if [[ -z "${BASE}" ]]; then
  BASE="$(mktemp -d "${TMPDIR:-/tmp}/nexo-nuget-org-restore-isol-XXXXXX")"
