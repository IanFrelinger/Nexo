...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        // Test 5: Verification Spec Execution
        Console.WriteLine("5. Testing Verification Spec Execution...");
        await TestVerificationSpecExecution();
        
        // Test 6: Run actual verification specs
        Console.WriteLine("6. Running Verification Specs...");
        await TestVerificationSpecs();
...
";
    ...
    static async Task TestVerificationSpecExecution()
    {
        try
        {
            // Simulate verification spec execution
            Console.WriteLine("  Simulating verification spec execution...");
            
            // Create test scenario
            var testDir = Path.Combine(Path.GetTempPath(), "nexo-verify-execution");
            Directory.CreateDirectory(testDir);
            
            // Simulate OFF mode execution
            Environment.SetEnvironmentVariable("NEXO_AI_MODE", "off");
            await Task.Delay(100); // Simulate work
            
            // Create output file
            var outputFile = Path.Combine(testDir, "outputs", "labels.csv");
            Directory.CreateDirectory(Path.GetDirectoryName(outputFile)!);
            File.WriteAllText(outputFile, "id,subject,label,confidence\n1,Test email,urgent,0.95");
            
            // Verify output exists
            if (File.Exists(outputFile))
            {
                var content = File.ReadAllText(outputFile);
                if (content.Contains("id,subject,label,confidence"))
                {
                    Console.WriteLine("  ✓ Verification spec execution test passed");
                }
                else
                {
                    Console.WriteLine("  ✗ Output file content validation failed");
                }
            }
            else
            {
        ...
        {
            Console.WriteLine($"  ✗ Verification spec execution test failed: {ex.Message}");
        }
    }
    
    static async Task TestVerificationSpecs()
    {
        try
        {
            // Test triage support spec
            var triageSpec = "../tests/specs/triage_support.verify.yaml";
            if (File.Exists(triageSpec))
            {
                Console.WriteLine("  Running triage_support.verify.yaml...");
                var exitCode = await SimpleVerifyRunner.RunAsync(triageSpec);
                if (exitCode == 0)
                {
                    Console.WriteLine("  ✓ Triage support verification passed");
                }
                else
                {
                    Console.WriteLine("  ✗ Triage support verification failed");
                }
            }
            else
            {
                Console.WriteLine("  ⚠ Triage support spec not found, skipping");
            }
            
            // Test PII redact spec
            var piiSpec = "../tests/specs/pii_redact.verify.yaml";
            if (File.Exists(piiSpec))
            {
                Console.WriteLine("  Running pii_redact.verify.yaml...");
                var exitCode = await SimpleVerifyRunner.RunAsync(piiSpec);
                if (exitCode == 0)
                {
                    Console.WriteLine("  ✓ PII redact verification passed");
                }
                else
                {
                    Console.WriteLine("  ✗ PII redact verification failed");
                }
            }
            else
            {
                Console.WriteLine("  ⚠ PII redact spec not found, skipping");
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"  ✗ Verification specs test failed: {ex.Message}");
        }
    }
}
