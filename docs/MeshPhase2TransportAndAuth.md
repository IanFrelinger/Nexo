...
Phase 2 tightens the **HTTP surface** used by the federated mesh program: **`/api/mesh`** (Phase 1 director) and **`POST /api/bricks/{id}/execute`** (remote brick delegation). It does **not** replace TLS or network ACLs; operators still terminate TLS at a reverse proxy or use Tailscale (see [TailscaleAndNexo.md](TailscaleAndNexo.md)).
...
## Pipeline order (`Nexo.API`)

1. Static files  
2. **`UseNexoMeshSecurity()`** — mesh token, brick token, body size, rate limit  
3. **`UseNexoApiKeyAuth()`** — existing global API key / bearer / basic  
...
```bash
export Nexo__Security__Mesh__MeshMutatingToken="$(openssl rand -hex 32)"
export Nexo__Security__AuthorizationMode=ApiKey
export Nexo__Security__ApiKey="$(openssl rand -hex 16)"
```
...
| Date | Change |
|------|--------|
| 2026-04-22 | Initial Phase 2 doc and `Nexo:Security:Mesh` options. |
