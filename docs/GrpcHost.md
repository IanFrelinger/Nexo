...
|------|-------|---------|-------|
| `ASPNETCORE_URLS` / `--urls` | env / command line | `http://127.0.0.1:5001` (code default in `Program.cs`, applied only when no URL is configured) | Loopback by default, like every other Nexo host. Widen deliberately: `ASPNETCORE_URLS=http://0.0.0.0:5001` inside a container. Do not put `Urls` in `appsettings.json` — it loads after the host configuration and would silently override `ASPNETCORE_URLS`. |
