...
- **Open fleet infrastructure cleanup:** removed `src/Nexo.Core.Application/Fleet/**` and `src/Nexo.Infrastructure/Fleet/**`; mesh-lab worker executor lives in open `Nexo.Infrastructure/MeshLab`; fleet unit tests moved to `commercial/tests/Nexo.Commercial.Tests.Fleet`.
