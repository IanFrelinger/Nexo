...
| **CLI** | **`Ashlar.CLI`** .NET tool (`PackAsTool`; **on nuget.org since `0.1.1`** — `dotnet tool install --global Ashlar.CLI --version 0.1.1`, or `--tool-path <dir>` for a project-local install; `--add-source <feed>` is needed only for a pre-release version you packed yourself) or **GHCR `nexo-cli`** image | Image tag or digest; CLI `--version` / package | **`docs/GettingStarted.md`** (`doctor`, `pipeline`) |
...
**Publication status:** **`0.1.1` is live on nuget.org** — published 2026-09-01 by `release.yml` via Trusted Publishing/OIDC, with SPDX SBOMs: the whole `Ashlar.*` graph including `Ashlar.Hosting.Bundle`, `Ashlar.Authoring`, `Ashlar.Sdk`, `Ashlar.Client` and the `Ashlar.CLI` tool. The NuGet rows above therefore need no feed setup at all: **`docs/ConsumingFromNuGet.md`** is the package-only recipe, and [github.com/IanFrelinger/ashlar-release-manager](https://github.com/IanFrelinger/ashlar-release-manager) is the out-of-tree proof (its CI restores from nuget.org and nothing else). Local folder feeds (`nuget-local-pack-consumer`, `scripts/verify-standalone-brick-authoring.sh`) remain the way to exercise a **pre-release** version you packed yourself, and a **`ProjectReference`** into `src/` (the **Source / monorepo** row) still needs no feed either.
