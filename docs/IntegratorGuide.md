...
After each **tagged release**, add a row for that **semver** (packages + `ashlar-cli` / `ashlar-api` digest pins) so integrators can copy known-good pins. Keep **`docs/DistributionModels.md`** in sync when channels or golden paths change.
