...
4. Run `dotnet run --project application/src/Nexo.CLI -- validate` before publishing.
...
|-----------|------------------|--------|
| **Monorepo / source (`master`)** | **SDK 8.x** (`global.json`); **API, CLI, libraries, and tests** target **`net8.0`** unless a package documents a retained **`netstandard2.0`** compatibility target (see **`docs/architecture/DotnetVersions.md`**) | You build from **`application/Nexo.Application.sln`** or **`Nexo.sln`**; no single “repo version” until you tag. |
...
- **CLI or doctor failures after dependency changes**: Run `dotnet run --project application/src/Nexo.CLI -- doctor --fix --dry-run` to see planned remediation without executing commands, then `doctor --fix --yes` if appropriate.
...
- **Release gate bundle**: Use `dotnet run --project application/src/Nexo.CLI -- ci release-bundle --profile full` for an extended step list; reports land under `.nexo/release-bundle/last-run/`.
