...
|-----------|------------------|--------|
| **Monorepo / source (`master`)** | **SDK 10.x** (`global.json`); **CLI**, **API**, and shipped hosts target **`net10.0`**; **libraries** multi-target **`net8.0;net10.0`**; tests use **`net8.0`** / **`net10.0`** (see **`docs/architecture/DotnetVersions.md`**) | You build from **`application/Ashlar.Application.sln`** or **`Ashlar.sln`**; no single “repo version” until you tag. |
