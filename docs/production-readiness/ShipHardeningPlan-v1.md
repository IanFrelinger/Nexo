...
**Automation:** `make ship-gate-full`. PRs that touch BaseFramework smoke tests or `scripts/ship-gate-tier-b.sh` / `scripts/ship-gate-tier-c.sh` / `scripts/ship-gate-tier-d.sh` run B, C, and D. A stays dispatch-only.
...
| `SHIP_GATE_SKIP_TIER_D=1` | Skip the D tier entirely from `ship-gate-full` |
| `SHIP_GATE_VERSION=x.y.z` | Preflight version (default: canonical `VERSION` file; must be valid semver) |
| `SHIP_GATE_BUNDLE_PROFILE=quick\|default\|full` | Release bundle profile (default `quick`) |
