...
Each run creates a JSON daily file in `ASHLAR_DAILIES_PATH` (`/data/dailies` in compose).

## 3) Remote access and hardening

### Basic checklist (avoid the biggest risks)

The Ashlar API is **powerful and open by default unless you enable built-in auth** — anyone who can open the HTTP port can call `/api/director/run`, `/api/orchestrate`, etc. You do not need “enterprise security” for a home lab, but these steps avoid common foot-guns:

1. **Shrink who can reach the port** — Prefer `127.0.0.1` / SSH or VPN to the host. Use `-ListenLan` / `--listen-lan` only on **Wi‑Fi you trust**; never on café/hotel networks. For **Tailscale**, see **`docs/TailscaleAndAshlar.md`** (ACLs + `Ashlar__Security__ExposureProfile=Tailnet` so logs and the portal advisory match your intent).
...
7. **Backups** — Back up the **dailies** volume or directory (`ASHLAR_DAILIES_PATH`) if you care about history.
...
- Back up the `ashlar-dailies` Docker volume.

### Exposing Ashlar on the public Internet

`Ashlar.API` supports optional built-in API auth layers, but they are not enabled by default and do not replace edge hardening. Treat **any** Internet reachability as “full access to whatever Ashlar can do on that host,” unless you configure auth and add a **front door**.

**Recommended patterns (pick one):**

| Approach | Idea | Tradeoff |
|----------|------|----------|
| **Private network only** | **Tailscale**, **WireGuard**, **Tailscale subnet router**, corporate VPN. Ashlar stays on `127.0.0.1` or a private IP; **no** public `8080`. | Best security/cost ratio for individuals; users need the VPN app. |
| **TLS + auth reverse proxy** | **Caddy**, **nginx**, **Traefik**, or **Envoy** terminates **HTTPS** on **443**, enforces **Basic Auth**, **OAuth2/OIDC**, or **mTLS**; proxies to `http://127.0.0.1:8080` only. | You operate certs (Let’s Encrypt via Caddy/nginx) and identity; Ashlar stays unmodified. |
...
- Port-forward **plain HTTP 8080** from your router to Ashlar (no TLS, no auth).
...
- **Separate machine or VM** for Internet-facing Ashlar from your main dev PC.
