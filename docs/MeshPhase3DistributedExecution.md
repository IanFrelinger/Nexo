...
- **`X-Nexo-Correlation-Id`** — If the client omits it on **`/api/mesh/*`** or **`POST /api/bricks/*/execute`**, **`MeshCorrelationMiddleware`** generates one and **echoes** it on the response. The value is also used when creating/updating mesh tasks if **`CorrelationId`** is not set in the JSON body.
