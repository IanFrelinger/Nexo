# ashlar — terminal style guide

The CLI is the brand's fourth surface (after README, NuGet, GitHub). Same palette, same node language, adapted to a dark terminal. `AshlarConsole.cs` implements everything below.
...
- Honor `NO_COLOR` and output redirection (both built into `AshlarConsole.UseColor`)
- Truecolor first; the 256 column above is the graceful-degradation map
- Windows: call `AshlarConsole.EnableVirtualTerminal()` once at startup for legacy conhost; Windows Terminal needs nothing
