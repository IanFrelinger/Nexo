...
`survivingMutantIds`, `gatesPassed[]` (name, version, configuration), `inputs[]` (kind, id, hash —
the witness, and the `compile-options` the program was judged under), `signature`, `gate` and
