...
- The public-API analyzer runs in **every** build of the covered projects with `TreatWarningsAsErrors`; it is not a separate job and cannot be skipped by path filters. `cert-gate` and `kernel-gate` (`.github/workflows/`) build `Nexo.Abstractions`, `Nexo.Brick.Contracts` and `Nexo.Client`; the `Nexo.sln` builds (`cross-platform-tests`, `production-readiness-gate-v1`, `runtime-release-gate`, release pack) build all five. An unreviewed public-API change fails them, and fails the local `dotnet build` that produced it.
