...
The **default** path is **containers + CLI**: develop inside the **Dev Container**, use the published CLI image, or build local Docker images / compose stacks from this checkout. If you cannot use Docker at all, use **`scripts/setup/*`** on a machine with **.NET SDK 8** (no separate repo “installer” scripts). See `README.md` for the full map.
...
1. Pick one startup lane (**Dev Container**, **published CLI image**, **quickstart image**, or **native** escape hatch).
...
- **Default:** Docker (Desktop or Engine) and Git. You do **not** need a host .NET SDK for the Dev Container, quickstart image, or published CLI image.
...
Run **`doctor`** on your workstation or inside the **Dev Container** for a full dependency check; CI validates the CLI image with **`--help`** and **`pipeline validate --help`** instead (see **`docs/DistributionModels.md`**). Version-pinned image tags are tracked in **`docs/Roadmap.md`** for the release sprint.
