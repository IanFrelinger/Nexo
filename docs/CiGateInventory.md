...
| Class | Count | Meaning |
| --- | --- | --- |
| Runs on `pull_request` | 31 | 4 unfiltered (`cert-gate`, `layer-boundary`, `uat-gate`, `composition-mesh-gate`), 26 path-filtered (including `products-gate`, Release Manager validation, compat/dr/perf/production-readiness/ship-gate, `ingress-unit-gate`, `onboarding-docs-guard`, `pack-hosting-graph-alignment`, `onboarding-quickstart-gate`, `environment-setup-gate-v1`, `optimize-agent-cluster-gate`, `runtime-release-gate`, `installer-bruteforce-gate`), 1 label-driven (`release-staging-on-label`) |
| Push- and/or schedule-driven, plus `workflow_dispatch` | 9 | Post-merge / scheduled signal; never blocks a PR. |
...
| --- | --- | --- | --- |
| `cert-gate.yml` | Cert gate / `cert-gate` | **every PR** (no paths) — analyzer 56 + contracts 18 + enrolled conventions 66 counted; main Infra filter excludes convention tests and uses list-tests plus collapse floor **447** | push `master`, dispatch — **required** |
...
| `ship-gate.yml` | Ship Gate / `ship-gate` | paths: BaseFramework smoke tests, `scripts/ship-gate-tier-b.sh` — PR runs counted Tier B smoke (9) + ProdStyle; A/C stay dispatch-only; D (dispatch) always runs `ci release-bundle` | dispatch |
...
| `rc-gate.yml` | RC Gate | `master`/`main`; RC docs + scripts; **monthly schedule**; automatic C produces `ci release-bundle` then fail-closes on missing/FAIL bundle; Tier D refuses missing/unauthenticated `gh` (exit 2) and sets `GH_TOKEN` |
