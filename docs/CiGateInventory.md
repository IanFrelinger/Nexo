...
| --- | --- | --- | --- |
| `cert-gate.yml` | Cert gate / `cert-gate` | **every PR** (no paths) — analyzer 56 + contracts 18 + enrolled conventions 75 counted; main Infra filter excludes convention tests and uses list-tests plus collapse floor **447** | push `master`, dispatch — **required** |
...
| `ops-gate.yml` | Ops Gate / `ops-gate` | paths: dogfood tests, `scripts/ops-gate-tier-a.sh`, `scripts/ops-gate-tier-b.sh`, counted wrapper — PR runs counted Tier A dogfood 1–6 (floor 6) plus Tier B 7–9 + IPC (floor 4); C/E stay dispatch-only; D needs Docker plus proof flags | dispatch |
