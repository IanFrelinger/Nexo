...
| Class | Count | Meaning |
| --- | --- | --- |
| Runs on `pull_request` | 27 | 4 unfiltered (`cert-gate`, `layer-boundary`, `uat-gate`, `composition-mesh-gate`), 22 path-filtered (including `products-gate`, Release Manager validation, compat/dr/perf/production-readiness/ship-gate, `ingress-unit-gate`, `onboarding-docs-guard`, `pack-hosting-graph-alignment`, `onboarding-quickstart-gate`), 1 label-driven (`release-staging-on-label`) |
| Push- and/or schedule-driven, plus `workflow_dispatch` | 12 | Post-merge / scheduled signal; never blocks a PR. |
...
| --- | --- | --- | --- |
| `cert-gate.yml` | Cert gate / `cert-gate` | **every PR** (no paths) — analyzer 56 + contracts 18 + enrolled conventions 59 counted; main Infra filter excludes convention tests and uses list-tests plus collapse floor **447** | push `master`, dispatch — **required** |
