...
| --- | --- | --- | --- |
| `cert-gate.yml` | Cert gate / `cert-gate` | **every PR** (no paths) — analyzer 56 + contracts 18 + enrolled conventions 77 counted; main Infra filter excludes convention tests and uses list-tests plus collapse floor **447** | push `master`, dispatch — **required** |
...
| `security-gate.yml` | Security Gate / `security-gate` | paths: Trust/Security sources and tests, `scripts/security-gate*.sh`, `Makefile` | PR: Tiers A–D plus E host suite (52); D fails when `dotnet list` cannot run and sets `SECURITY_GATE_STRICT_SUPPLY_CHAIN=1`; E container dispatch-only (release-manager `security` lane runs full A–E with Docker) |
...
| `ops-gate.yml` | Ops Gate / `ops-gate` | paths: dogfood tests, `scripts/ops-gate-tier-a.sh`, `scripts/ops-gate-tier-b.sh`, `scripts/ops-gate-tier-c.sh`, counted wrapper — PR runs counted A (floor 6), B (floor 4), and C closed-loop (floor 1); E stays dispatch-only; D needs Docker plus proof flags | dispatch |
