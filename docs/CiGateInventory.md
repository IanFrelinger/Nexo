...
| --- | --- | --- | --- |
| `cert-gate.yml` | Cert gate / `cert-gate` | **every PR** (no paths) — analyzer 56 + contracts 18 + enrolled conventions 113 counted; main Infra filter excludes convention tests and uses list-tests plus collapse floor **447** | push `master`, dispatch — **required** |
...
| `distribution-matrix-gate.yml` | Distribution Matrix Gate / 7 jobs | paths: same broad list as push (Dockerfiles, pack/verify scripts, Ashlar.API/CLI, Client/Sdk/Hosting.Bundle/Authoring/Brick.Contracts, samples, VirtualProduction tests) — counted IAshlarClient floor 1 + counted standalone brick authoring floor 1 | push (broad paths), weekly schedule, dispatch |
...
| `ship-gate.yml` | Ship Gate / `ship-gate` | paths: BaseFramework smoke tests, `scripts/ship-gate-tier-b.sh`, `scripts/ship-gate-tier-c.sh`, `scripts/ship-gate-tier-d.sh` — PR runs counted Tier B smoke (9) + ProdStyle and fails closed if `doctor --json` is non-zero, plus pack+consume preflight and `ci release-bundle`; A stays dispatch-only | dispatch |
