...
| `Nexo.Commercial.Fleet.Api` | seeded from `Nexo.API` `/api/mesh` fleet/task/knowledge endpoints | Commercial endpoint extension for fleet director HTTP APIs; open endpoints remain temporarily until commercial host wiring moves. |
