...
3. **CLI** — **`nexo mesh hub list`** prints peers from discovery; **`nexo mesh hub health --url …`** probes **`GET /health`**. **`nexo mesh director`** (get/post/patch) calls a remote **`Nexo.API`** using **`NEXO_MESH_DIRECTOR_BASE_URL`**, **`NEXO_MESH_API_KEY`**, and optional **`NEXO_MESH_MUTATING_TOKEN`**.
...
- **`Nexo.Tests.CLI`** — `MeshDirectorCommandUriTests`
