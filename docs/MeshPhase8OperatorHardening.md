...
3. **CLI** — **Open:** **`nexo mesh peers`** lists local `instances.json` peers; **`nexo mesh health --url …`** probes **`GET /health`**. **Commercial:** **`dotnet run --project commercial/src/Nexo.Commercial.MeshDirector -- director list-nodes`**, **`director health`**, **`director admit`**, etc. call the fleet director using **`NEXO_MESH_DIRECTOR_BASE_URL`**, **`NEXO_MESH_API_KEY`**, and optional **`NEXO_MESH_MUTATING_TOKEN`**.
