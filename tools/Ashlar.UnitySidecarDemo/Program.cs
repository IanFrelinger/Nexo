...
namespace Ashlar.UnitySidecarDemo;
...
internal static class Program
{
    ...
    private static async Task<int> Main(string[] args)
    {
        ...
        if (repoRoot == null)
        {
            Console.Error.WriteLine("Could not locate repo root (Ashlar.sln not found).");
            return 1;
        }
    ...
    private static async Task<int> RunDemoAsync(string repoRoot, Dictionary<string, string> options)
    {
        ...
        var dogfoodResult = await RunCommandCaptureAsync(
            "dotnet",
            "run --project application/src/Ashlar.CLI --no-build -- dogfood block1 --format-json",
            repoRoot);
    ...
    private static async Task<int> GenerateAsync(string repoRoot, Dictionary<string, string> options)
    {
        ...
        var ashlarResult = await RunCommandCaptureAsync(
            "dotnet",
            $"run --project application/src/Ashlar.CLI --no-build -- chat --prompt \"{EscapeForShell(prompt)}\"",
            repoRoot,
            env: new Dictionary<string, string> { ["ASHLAR_ALLOW_MOCK"] = "1" });
        ...
        var manifest = new
        {
            ...
            ashlarChatExitCode = ashlarResult.ExitCode,
            ashlarChatSummary = Truncate(ashlarResult.StdOut.Trim(), 600)
        };
        ...
        Console.WriteLine(JsonSerializer.Serialize(payload, JsonOptions()));
        return ashlarResult.ExitCode == 0 ? 0 : 1;
    }
...
""";
    ...
    private static void PrintHelp()
    {
        ...
        help.AppendLine("Ashlar.UnitySidecarDemo");
        help.AppendLine();
        ...
        help.AppendLine("  dotnet run --project tools/Ashlar.UnitySidecarDemo -- generate --prompt \"add a dash ability\"");
        help.AppendLine("  dotnet run --project tools/Ashlar.UnitySidecarDemo -- validate");
        help.AppendLine("  dotnet run --project tools/Ashlar.UnitySidecarDemo -- run-demo --prompt \"add a health pickup system\"");
        help.AppendLine("  dotnet run --project tools/Ashlar.UnitySidecarDemo -- supervise --game \"build a coop dungeon crawler\" --iterations 2");
