...
# Run all six release audit sub-agents in isolated worktrees. The requested version
# must match the root VERSION file; the coordinator never publishes or tags.
release-manager-audit:
	python3 scripts/autonomous-release-manager.py $(if $(VERSION),--version "$(VERSION)",)

release-manager-validate:
	python3 scripts/autonomous-release-manager.py --validate-only
...
kernel-gate-tier-a:
	bash scripts/kernel-gate-tier-a.sh
...
# Ingress unit suites (AwsSns + DynamoDb) that used to be sln-only / UNOWNED.
ingress-unit-gate:
	bash scripts/ingress-unit-gate.sh

mcp-a2a-gate:
	bash scripts/mcp-a2a-gate.sh

production-readiness-gate-v1-tests:
	bash scripts/production-readiness-gate-v1-tests.sh
